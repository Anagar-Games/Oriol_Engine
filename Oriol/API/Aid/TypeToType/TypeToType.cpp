...
namespace OL
{
    ...
    std::string WstringToString(const std::wstring& wstr_a)
    {
        ...
        int size_ = WideCharToMultiByte(CP_UTF8,
                                        0,
                                        wstr_a.c_str(),
                                        -1,
                                        nullptr,
        ...
        std::string result;
        for (wchar_t wc : wstr_a) {
            if (wc <= 0x7F) {
                result += static_cast<char>(wc);
            } else {
                // For non-ASCII characters, use a simple conversion
    ...
    int ExtractNumber(const std::string& mem_info_a)
    {
        ...
        size_t number_start_ = mem_info_a.find_first_of("0123456789",
                                                        colon_pos_ + 1);
        ...
        size_t number_end_ = mem_info_a.find_first_not_of("0123456789",
                                                          number_start_);
        ...
        try
        {
            ...
            mem_info_a.substr(number_start_,
                              number_end_ - number_start_);
