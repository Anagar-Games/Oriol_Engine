...
#ifndef OL_SYMBOLTABLE_HPP
#define OL_SYMBOLTABLE_HPP
...
namespace OL
{
    unsigned int StringDistance(const std::string& a_a, const std::string& b_a);
    ...
    {
    public:
        using OnOverrideProc = std::function<bool(SymbolType& prev_symbol_)>;
        using OnReleaseProc = std::function<void(const SymbolType& symbol_)>;
                      ...
                      bool throw_on_failure_a = true)
        {
            ...
            if (ident_a.empty())
            {
                sym_table_anonymous_.back().push_back({symbol_a, ScopeLevel()});
            } else
            {
                ...
                if (it_ != sym_table_.end() && !it_->second.empty())
                {
                    ...
                    if (entry_.symbol_ && entry_.scope_level_ == ScopeLevel())
                    {
        ...
        SymbolType Find(const SearchPredicateProc& search_predicate_a) const
        {
    ...
    public:
        ...
        FunctionDecl* FetchFunctionDecl(bool throw_on_failure_a = true) const;
