...
namespace OL
{
    GLSLPreProcessor::GLSLPreProcessor(
			IncludeHandler& include_handler_a, Log* log_a) : 
    ...
    bool GLSLPreProcessor::OnDefineMacro(const Macro& macro_a)
    {
        if (!macro_a.std_macro_)
        {
            ...
            if (underscore_pos_ != std::string::npos)
            {
                ...
                source_area_.Offset(static_cast<unsigned int>(underscore_pos_));
    ...
    bool GLSLPreProcessor::OnSubstitueStdMacro(const Token& ident_tkn_a,
        ...
        if (Is(Tokens::Ident))
        {
			...
			else
            {
                if (profile_ == "compatibility")
                    ...
                    Error(R_InvalidGLSLVersionProfile(profile_), true, false);
            }
        }
    ...
    void GLSLPreProcessor::ParseDirectiveExtension()
    {
        ...
        if (behavior_ != "enable" && behavior_ != "require"
            ...
            Error(R_InvalidGLSLExtensionBehavior(behavior_), true, false);

        Out() << "#extension " << extension_ << " : " << behavior_;
    }

    bool GLSLPreProcessor::VerifyVersionNo(const int* valid_versions_a) const
    {
