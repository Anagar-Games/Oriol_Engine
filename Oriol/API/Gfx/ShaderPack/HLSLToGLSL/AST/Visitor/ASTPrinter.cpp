...
namespace OL
{
    ...
    template <>
    inline std::string MemberToString<Identifier>(const Identifier& member_a)
    {
        return member_a.Original();
    }

    template <>
    inline std::string MemberToString<std::string>(const std::string& member_a)
    {
        return member_a;
    }
...
#define ADD_PRINTABLE_MEMBER(MEMBER)                                           \
    ...
    IMPLEMENT_VISIT_PROC(Register)
    {
        PushPrintable(ast_a, WriteLabel("Register"));
        {
            Printable(ast_a,
                      "registerType",
                      std::string(1,
                                  RegisterTypeToChar(ast_a->register_type_)));
    ...
    IMPLEMENT_VISIT_PROC(LayoutStmnt)
    {
        ...
        {
            VISIT_MEMBER(attribs_);
            Printable(ast_a, "isInput", (ast_a->is_input_ ? "true" : "false"));
            Printable(ast_a,
                      "isOutput",
...
#undef ADD_PRINTABLE_MEMBER
    ...
    void ASTPrinter::Print(const PrintableTree& tree_a, std::ostream& output_a)
    {
        ...
        {
            for (std::size_t i_ = 0; i_ + 1 < last_sub_node_stack_.size(); ++i_)
            {
