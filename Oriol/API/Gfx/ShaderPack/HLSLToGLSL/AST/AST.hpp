...
namespace OL
{
    ...
    using VarDeclIteratorFunctor = std::function<void(VarDeclPtr& var_decl_a)>;
    ...
    using FindPredicateConstFunctor = std::function<bool(const Expr& expr_a)>;

    using MergeExprFunctor = std::function<ExprPtr(const ExprPtr& expr0_a,
                                                   const ExprPtr& expr1_a)>;
...
#define FLAG(IDENT, INDEX) IDENT = (1u << (INDEX))
    ...
    struct Expr : public TypedAST
    {
        ...
        virtual bool IsTrivialCopyable(unsigned int max_tree_depth_a = 3) const;

        virtual const Expr* Find(const FindPredicateConstFunctor& predicate_a,
                                 unsigned int flags_a = SearchAll) const;

        const Expr* FindFirstOf(const Types expr_type_a,
                                unsigned int flags_a = SearchAll) const;

        const Expr* FindFirstNotOf(const Types expr_type_a,
                                   unsigned int flags_a = SearchAll) const;
    ...
    struct Program : public AST
    {
        ...
        void RegisterIntrinsicUsage(const Intrinsic intrinsic_,
    ...
    struct TypeSpecifier : public TypedAST
    {
        ...
        std::set<StorageClass> storage_classes_;
    ...
    struct StructDecl : public Decl
    {
        ...
        std::size_t MemberVarToIndex(const VarDecl* var_decl_a,
                                     bool include_base_structs_a = true) const;
    ...
    struct FunctionDecl : public Decl
    {
        ...
        bool MatchParameterWithTypeDenoter(std::size_t param_index_a,
    ...
    struct InitializerExpr : public Expr
    {
        ...
        ExprPtr FetchSubExpr(const std::vector<int>& array_indices_a) const;
