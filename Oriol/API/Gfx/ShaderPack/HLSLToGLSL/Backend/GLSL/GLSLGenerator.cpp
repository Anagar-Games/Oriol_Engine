...
namespace OL
{
    ...
    void GLSLGenerator::GenerateCodePrimary(Program& program_a,
    ...
    bool GLSLGenerator::IsWrappedIntrinsic(const Intrinsic intrinsic_a) const
    {
    ...
    const std::string* GLSLGenerator::BufferTypeToKeyword(
            ...
            const AST* ast_a)
    {
        if (auto keyword_ = BufferTypeToGLSLKeyword(buffer_type_a,
                                                    IsVKSL(),
                                                    UseSeparateSamplers()))
            return keyword_;
        else
    ...
    void GLSLGenerator::ReportOptionalFeedback()
    {
            ...
            && IsVertexShader())
        {
            ...
            {
                if (loc_.second > 1)
                    Warning(R_MultiUseOfVertexSemanticLocation(loc_.first,
                                                               loc_.second));
            }
        }
    }
    ...
    void GLSLGenerator::ErrorIntrinsic(const std::string& intrinsic_name_a,
                                       const AST* ast_a)
    {
        Error(R_FailedToMapToGLSLKeyword(R_Intrinsic(intrinsic_name_a)), ast_a);
    }

    int GLSLGenerator::GetNumBindingLocations(const TypeDenoter* type_denoter_a)
    {
        if (!type_denoter_a)
            return -1;
    ...
    int GLSLGenerator::GetBindingLocation(const TypeDenoter* type_denoter_a,
                                          bool input_a)
    {
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(Program)
    {
        ...
        if (separate_shaders_ && version_out_ > OutputShaderVersion::GLSL140)
    ...
    IMPLEMENT_VISIT_PROC(StructDecl)
    {
                              ...
                              | StructDecl::IsShaderOutput))
        {
            ...
            {
                if (auto struct_decl_args_ = reinterpret_cast<StructDeclArgs*>(
                            args_a))
    ...
    IMPLEMENT_VISIT_PROC(FunctionDecl)
    {
        ...
        if (!ast_a->flags_(AST::IsReachable))
        {
            if (WarnEnabled(Warnings::Basic)
                ...
                Warning(R_InvalidControlPathInUnrefFunc(ast_a->ToString()),
                        ast_a);
            return;
        }
    ...
    IMPLEMENT_VISIT_PROC(VarDeclStmnt)
    {
        ...
        if (ast_a->type_specifier_->HasAnyStorageClassOf({StorageClass::Static})
        ...
        PushVarDeclStmnt(ast_a);
        {
            ...
            if (!InsideStructDecl())
            {
                WriteInterpModifiers(ast_a->type_specifier_->interp_modifiers_,
                                     ast_a);

                WriteStorageClasses(ast_a->type_specifier_->storage_classes_,
                                    ast_a);
            }
    ...
    IMPLEMENT_VISIT_PROC(CallExpr)
    {
        ...
        else if (ast_a->intrinsic_ == Intrinsic::InterlockedCompareExchange)
...
#undef IMPLEMENT_VISIT_PROC
            ...
            const ShaderInput& input_desc_a)
    {
        ...
        struct_analyzer_.MarkStructsFromEntryPoint(*GetProgram(),
    ...
    void GLSLGenerator::PreProcessExprConverterPrimary()
    {
        ...
        {
            converter_flags_.Remove(ExprConverter::ConvertVectorSubscripts);
            converter_flags_.Remove(ExprConverter::ConvertInitializerToCtor);
        }
            ...
            const ShaderInput& input_desc_a)
    {
        ...
        ref_analyzer_.MarkReferencesFromEntryPoint(*GetProgram(),
                                                   input_desc_a.shader_target_);
    }
    ...
    void GLSLGenerator::WriteProgramHeaderVersion()
    {
        int version_number_ = (static_cast<int>(version_out_))
                              & static_cast<int>(OutputShaderVersion::GLSL);
            ...
            const Program::LayoutTessEvaluationShader& layout_a)
    {
        WriteLayoutGlobalIn({
                ...
                [&]() {
                    if (IsAttributeValuePartitioning(layout_a.partitioning_))
                    {
                        if (auto keyword_ = AttributeValueToGLSLKeyword(
                                    layout_a.partitioning_))
                            Write(*keyword_);
                        else
                            Error(R_FailedToMapToGLSLKeyword(R_Partitioning,
                                                             R_TessSpacing));
                    }
                },
            ...
            const Program::LayoutGeometryShader& layout_a)
    {
        WriteLayoutGlobalIn({
                [&]() {
                    if (layout_a.input_primitive_ == PrimitiveType::Undefined)
            ...
            const Program::LayoutFragmentShader& layout_a)
    {
        ...
        {
            WriteLayoutGlobalIn({[&]() { Write("early_fragment_tests"); }});
            layouts_written_ = true;
        }
    ...
    void GLSLGenerator::WriteLayout(
            const std::initializer_list<LayoutEntryFunctor>& entry_functors_a)
    {
            ...
            const std::vector<RegisterPtr>& slot_registers_a)
    {
        if (explicit_binding_
            && (!IsESSL() || version_out_ >= OutputShaderVersion::ESSL310))
        {
            if (auto slot_register_ = Register::GetForTarget(slot_registers_a,
                                                             GetShaderTarget()))
                Write("binding = " + std::to_string(slot_register_->slot_));
        }
    }
    ...
    void GLSLGenerator::WriteLocalInputSemantics(FunctionDecl* entry_point_a)
    {
        ...
        {
            const auto& type_den_ =
                    param_->type_specifier_->GetTypeDenoter()->GetAliased();
    ...
    void GLSLGenerator::WriteLocalInputSemanticsVarDecl(VarDecl* var_decl_a)
    {
            ...
            && struct_decl_a->flags_(StructDecl::IsShaderInput))
        {
            ...
            BeginSep();
            {
                ...
                if (param_var_->array_dims_.empty())
                {
                    struct_decl_a->ForEachVarDecl([&](VarDeclPtr& var_decl_) {
                        BeginLn();
                        {
                            ...
                            Separator();
                            Write(" = ");
                            WriteVarDeclIdentOrSystemValue(var_decl_.get());
                            Write(";");
                        }
                        EndLn();
                    });
                } else if (param_var_->array_dims_.size() == 1)
                {
                    ...
                    Error(R_TooManyIndicesForShaderInputParam, param_var_);
            }
            EndSep();
        }
    }

    void GLSLGenerator::WriteGlobalInputSemantics(FunctionDecl* entry_point_a)
    {
    ...
    void GLSLGenerator::WriteGlobalInputSemanticsVarDecl(VarDecl* var_decl_a)
    {
        BeginLn();
        {
            ...
            {
                if (WarnEnabled(Warnings::Basic) && !interp_modifiers_.empty())
            ...
            } else
            {
                    ...
                    || (IsESSL() && IsVertexShader()))
                {
                    ...
                    {
                        auto it_ = vertex_semantics_map_.find(
                                ToCiString(var_decl_a->semantic_.ToString()));
                    ...
                    {
                        WriteLayout({[&]() {
                            Write("location = " + std::to_string(location_));
                        }});
    ...
    void GLSLGenerator::WriteLocalOutputSemantics(FunctionDecl* entry_point_a)
    {
    ...
    void GLSLGenerator::WriteGlobalOutputSemantics(FunctionDecl* entry_point_a)
    {
        ...
        if (IsFragmentShader() && version_out_ > OutputShaderVersion::GLSL120)
        {
            ...
            if (semantic_.IsValid())
            {
                if (semantic_.IsSystemValue())
                {
                                ...
                                semantic_))
                    {
                        ...
                        Error(R_FailedToMapToGLSLKeyword(R_OutputSemantic),
                              entry_point_);
    ...
    void GLSLGenerator::WriteGlobalUniformsParameter(VarDeclStmnt* param_a)
    {
        BeginLn();
        {
            Write("uniform ");
    ...
    void GLSLGenerator::WriteObjectExprIdent(const ObjectExpr& object_expr_a,
                                             bool write_prefix_a)
    {
            ...
            Decl* symbol_a)
    {
        ...
        if (auto var_decl_ = symbol_a->As<VarDecl>())
        {
            ...
            {
                semantic_keyword_ = SystemValueToKeyword(var_decl_->semantic_);
            }
        }
        ...
        {
            if (auto array_expr_ = object_expr_a.prefix_expr_
                                           ->FindFirstNotOf(
                                           ...
                                           ->As<ArrayExpr>())
            {
                ...
                Error(R_MissingArrayPrefixForIOSemantic(object_expr_a.ident_),
                      &object_expr_a);
    ...
    void GLSLGenerator::WriteStorageClasses(
            const std::set<StorageClass>& storage_classes_a,
            const AST* ast_a)
    {
        for (auto storage_ : storage_classes_a)
        {
            ...
            const TypeDenoterPtr& type_denoter_a)
    {
            ...
            && type_denoter_a->IsMatrix())
        {
            const auto common_storage_layout_ =
                    GetUniformBufferDeclStack().back()->common_storage_layout_;
        ...
        if (type_modifiers_a.find(TypeModifier::Const)
            != type_modifiers_a.end())
        {
            if ((IsGLSL() && version_out_ >= OutputShaderVersion::GLSL420)
                || IsVKSL())
                Write("const ");
        }
    }
                               ...
                               type_denoter_a.As<BufferTypeDenoter>())
            {
                ...
                if (buffer_type_ == BufferType::Undefined)
                {
                    if (auto buffer_decl_ = buffer_type_den_->buffer_decl_ref_)
                        ...
                        Error(R_MissingRefInTypeDen(R_BufferTypeDen), ast_a);
                }
                               ...
                               type_denoter_a.As<SamplerTypeDenoter>())
            {
                ...
                if (sampler_type_ == SamplerType::Undefined)
                {
                    if (auto sampler_decl_ =
                        ...
                        Error(R_MissingRefInTypeDen(R_SamplerTypeDen), ast_a);
                }

                if (!IsSamplerStateType(sampler_type_) || UseSeparateSamplers())
                {
    ...
    void GLSLGenerator::WriteFunctionEntryPoint(FunctionDecl* ast_a)
    {
        ...
        WriteScopeOpen();
        {
            ...
            {
                if (auto patch_const_func_ = GetProgram()
    ...
    void GLSLGenerator::WriteFunctionSecondaryEntryPoint(FunctionDecl* ast_a)
    {
        if (ast_a->IsForwardDecl())
            return;
                                               ...
                                               std::size_t num_args_max_a)
    {
        ...
        if (num_args_ < num_args_min_a || num_args_ > num_args_max_a)
            Error(R_InvalidIntrinsicArgCount(func_call_a->ident_), func_call_a);
    }
    ...
    void GLSLGenerator::WriteCallExprStandard(CallExpr* func_call_a)
    {
        if (func_call_a->intrinsic_ != Intrinsic::Undefined)
        {
            ...
            {
                if (auto keyword_ =
                            IntrinsicToGLSLKeyword(func_call_a->intrinsic_,
                                                   IsGLSL120OrESSL100()))
                    Write(*keyword_);
                else
    ...
    void GLSLGenerator::WriteCallExprIntrinsicMul(CallExpr* func_call_a)
    {
        ...
        Write("(");
        {
            WriteMulArgument(func_call_a->arguments_[1]);
            Write(" * ");
            WriteMulArgument(func_call_a->arguments_[0]);
        }
        Write(")");
    }
        ...
        if (auto base_type_den_ = type_den_.As<BaseTypeDenoter>())
        {
            ...
            if (base_type_den_->IsVector())
            {
                ...
                } else
                {
                    Visit(expr_);
                    Write(", ");
                    WriteLiteral("0", base_type_den_->data_type_, expr_.get());
                }
    ...
    void GLSLGenerator::WriteCallExprIntrinsicImageAtomic(CallExpr* call_expr_a)
    {
    ...
    void GLSLGenerator::WriteCallExprArguments(CallExpr* call_expr_a,
    ...
    void GLSLGenerator::WriteWrapperIntrinsics()
    {
        ...
        if (auto usage_ = program_l_->FetchIntrinsicUsage(Intrinsic::SinCos))
            WriteWrapperIntrinsicsSinCos(*usage_);

        if (program_l_->FetchIntrinsicUsage(
                    Intrinsic::GroupMemoryBarrierWithGroupSync)
            != nullptr)
            WriteWrapperIntrinsicsMemoryBarrier(Intrinsic::GroupMemoryBarrier,
                                                true);
        if (program_l_->FetchIntrinsicUsage(Intrinsic::DeviceMemoryBarrier)
            != nullptr)
            WriteWrapperIntrinsicsMemoryBarrier(Intrinsic::DeviceMemoryBarrier,
                                                false);
        if (program_l_->FetchIntrinsicUsage(
                    Intrinsic::DeviceMemoryBarrierWithGroupSync)
            != nullptr)
        ...
        if (program_l_->FetchIntrinsicUsage(
                    Intrinsic::AllMemoryBarrierWithGroupSync)
            != nullptr)
            WriteWrapperIntrinsicsMemoryBarrier(Intrinsic::AllMemoryBarrier,
                                                true);
        if (program_l_->FetchIntrinsicUsage(Intrinsic::F16toF32) != nullptr)
    ...
    void GLSLGenerator::WriteWrapperIntrinsicsLit(const IntrinsicUsage& usage_a)
    {
        (void)usage_a;
        BeginLn();
        {
            ...
            const MatrixSubscriptUsage& usage_a)
    {
        ...
        BeginLn();
        {
            ...
            Write(" ");
            Write(ExprConverter::GetMatrixSubscriptWrapperIdent(name_mangling_,
                                                                usage_a));
            Write("(");
    ...
    void GLSLGenerator::WriteBufferDeclTexture(BufferDecl* buffer_decl_a)
    {
            ...
            && !UseSeparateSamplers())
        {
            ...
            SamplerType shadow_sampler_type_ = SamplerTypeToShadowSamplerType(
                    sampler_type_);
        ...
        BeginLn();
        {
            WriteLayout({
                    [&]() {
                        if (!is_write_only_)
                        {
                            if (auto keyword_ = ImageLayoutFormatToGLSLKeyword(
            ...
            if (is_rw_buffer_
                && (is_write_only_
                    || image_layout_format_ == ImageLayoutFormat::Undefined))
                Write("writeonly ");
    ...
    void GLSLGenerator::WriteBufferDeclStorageBuffer(BufferDecl* buffer_decl_a)
    {
        ...
        BeginLn();
        {
            ...
            if (name_mangling_.rename_buffer_fields_)
            {
                ...
                Write(name_mangling_.temporary_prefix_ + buffer_decl_a->ident_);
    ...
    {
        if (UseSeparateSamplers()
            || !IsSamplerStateType(sampler_decl_a.decl_stmnt_ref_->type_denoter_
                                           ->sampler_type_))
        {
