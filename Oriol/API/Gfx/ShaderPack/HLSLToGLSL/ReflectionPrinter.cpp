...
namespace OL
{
    ...
    void ReflectionPrinter::PrintReflectionObjects(
            const std::vector<Reflection::StaticSamplerState>& sampler_states_a,
            const char* title_a)
    {
        IndentOut() << title_a << ':' << std::endl;
        ScopedIndent indent_ {indent_handler_};
        ...
        if (!sampler_states_a.empty())
        {
            for (const auto& sampler_ : sampler_states_a)
            {
                ...
                indent_handler_.IncIndent();
                {
                    ...
                    IndentOut()
                            << "AddressU       = " << ToString(desc_.address_u_)
                            << std::endl;
                    IndentOut()
                            << "AddressV       = " << ToString(desc_.address_v_)
                            << std::endl;
                    IndentOut()
                            << "AddressW       = " << ToString(desc_.address_w_)
                            << std::endl;
                    IndentOut() << "BorderColor    = { " << brd_col_[0] << ", "
                                << brd_col_[1] << ", " << brd_col_[2] << ", "
                                << brd_col_[3] << " }" << std::endl;
                    IndentOut()
                            << "ComparisonFunc = "
                            << ToString(desc_.comparison_func_) << std::endl;
                    IndentOut()
                            << "Filter         = " << ToString(desc_.filter_)
                            << std::endl;
                    IndentOut() << "MaxAnisotropy  = " << desc_.max_anisotropy_
                                << std::endl;
                    IndentOut() << "MaxLOD         = " << desc_.max_lod_
                                << std::endl;
                    IndentOut() << "MinLOD         = " << desc_.min_lod_
                                << std::endl;
                    IndentOut() << "MipLODBias     = " << desc_.mip_lod_bias_
                                << std::endl;
                }
                indent_handler_.DecIndent();
            }
    ...
    void ReflectionPrinter::PrintReflectionAttribute(
            ...
            const char* title_a)
    {
        if (num_threads_a.x_ > 0 || num_threads_a.y_ > 0
            || num_threads_a.z_ > 0)
        {
            IndentOut() << title_a << ':' << std::endl;
            ScopedIndent indent_ {indent_handler_};
