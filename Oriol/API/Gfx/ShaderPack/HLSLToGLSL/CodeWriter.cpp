...
namespace OL
{
    ...
    void CodeWriter::BeginLine()
    {
        ...
        if (!open_line_)
        {
            ...
            {
                if (line_separation_level_ > 0)
                    queued_separated_lines_.Current().indent_ = FullIndent();
                else
                    Out() << FullIndent();
            }
        }
                                ...
                                bool use_braces_a)
    {
        ...
        scope_option_stack_.push(
                {compact_a, end_with_semicolon_a, use_braces_a});
    }
    ...
    {
        return (!options_stack_.empty() ? options_stack_.top() : Options());
    }
