...
namespace OL
{
    ...
    static DataType IntrinsicReturnTypeToDataType(const IntrinsicReturnType t_a)
    {
        switch (t_a)
        {
            ...
            bool use_min_dimension_a = false)
    {
        if (major_arg_index_a < args_a.size())
        {
            ...
            for (std::size_t i_ = 0, n_ = args_a.size(); i_ < n_; ++i_)
            {
                ...
                {
                    common_type_denoter_ = TypeDenoter::FindCommonTypeDenoter(
    ...
    IntrinsicSignature::IntrinsicSignature(IntrinsicReturnType return_type_a,
                                           int num_args_a)
            ...
            const std::vector<ExprPtr>& args_a) const
    {
        ...
        if (return_type_ != IntrinsicReturnType::Void)
        {
            ...
            if (return_type_fixed_ != DataType::Undefined)
                return std::make_shared<BaseTypeDenoter>(return_type_fixed_);
    ...
    std::vector<TypeDenoterPtr> HLSLIntrinsicAdept::GetIntrinsicParameterTypes(
            const Intrinsic intrinsic_a,
            const std::vector<ExprPtr>& args_a) const
    {
    ...
    {
        static const HLSLIntrinsicsMap intrinsic_map_ = GenerateIntrinsicMap();
            ...
            const std::vector<ExprPtr>& args_a) const
    {
        ...
        const auto& arg0_type_den_ = args_a[0]->GetTypeDenoter()->GetAliased();
    ...
    {
        return std::make_shared<BaseTypeDenoter>(
                VectorDataType(BaseDataType(generic_type_denoter_a->data_type_),
                               4));
    }
        ...
        if (type0_->IsVector())
        {
            ...
            } else if (type1_->IsMatrix())
            {
                param_type_denoters_a.push_back(
                        TypeDenoter::FindCommonTypeDenoter(type0_, type1_));
            ...
            const Intrinsic intrinsic_a) const
    {
        if (args_a.size() != 1)
            RuntimeErr(R_InvalidIntrinsicArgCount(
                    intrinsic_a == Intrinsic::FirstBitLow ? "firstbitlow"
                                                          : "firstbithigh"));
        ...
        if (auto base_type0_ = type0_->As<BaseTypeDenoter>())
        {
            ...
            if (IsScalarType(base_data_type_) || IsVectorType(base_data_type_))
            {
    ...
    BaseTypeDenoterPtr HLSLIntrinsicAdept::GetGenericTextureTypeFromPrefix(
            const Intrinsic intrinsic_a,
            const TypeDenoterPtr& prefix_type_denoter_a) const
    {
        ...
        {
            auto generic_type_den_ = buffer_type_den_->GetGenericTypeDenoter();
            if (generic_type_den_->Type() == TypeDenoter::Types::Base)
            {
                return std::static_pointer_cast<BaseTypeDenoter>(
                        generic_type_den_);
            }
            RuntimeErr(R_ExpectedBaseTypeDen(generic_type_den_->ToString()));
        }
