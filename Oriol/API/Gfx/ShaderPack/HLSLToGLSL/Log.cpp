...
namespace OL
{
    ...
    static void PrintReport(const IndentReport& r_a, bool verbose_a)
    {
        ...
        {
            ConsoleManip::ScopedColor highlight_(Colors::Red | Colors::Intens);
        ...
        if (r_a.report_.HasLine())
        {
...

            {
                                  ...
                                  != std::string::npos)
                {
                    ...
                    {
                        ConsoleManip::ScopedColor highlight1_(Colors::Cyan);
                                     ...
                                     const std::string& headline_a = "")
    {
        if (!reports_a.empty())
        {
            if (!headline_a.empty())
            {
                auto s_ = std::to_string(reports_a.size()) + " " + headline_a;
    ...
    void StdLog::PrintAll(bool verbose_a)
    {
        ...
        PrintAndClearReports(data_->warnings_,
                             verbose_a,
                             (data_->warnings_.size() == 1 ? "WARNING"
                                                           : "WARNINGS"));
        PrintAndClearReports(data_->errors_,
                             verbose_a,
                             (data_->errors_.size() == 1 ? "ERROR" : "ERRORS"));
    }
} // namespace OL
