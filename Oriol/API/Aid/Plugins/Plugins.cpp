...
namespace OL
{
    ...
    void PluginManager::LoadPlugin(const std::string& path_a)
    {
        ...
        if (!create_plugin_)
        {
            ...
            throw std::runtime_error("Failed to find CreatePlugin symbol in: "
            + path_a);
        }
    ...
    const std::vector<std::unique_ptr<IPlugin>>& PluginManager::GetPlugins()
    const
    {
        // This cast is unsafe, consider redesigning the interface
    ...
    void* PluginManager::GetSymbolAddress(void* handle_a,
