...
namespace OL
{
    ...
    ProgramPtr GLSLParser::ParseSource(
			const SourceCodePtr& source_a, const NameMangling& name_mangling_a,
            const InputShaderVersion version_in_a,  bool enable_warnings_a)
    {
        (void)version_in_a;
    ...
    {
        return (IsBaseDataType() || Is(Tokens::Buffer) || Is(Tokens::Sampler)
    ...
    bool GLSLParser::IsRegisteredTypeName(const std::string& ident_a) const
    {
    ...
    ProgramPtr GLSLParser::ParseProgram(const SourceCodePtr& source_a)
    {
        ...
        while (true)
        {
            ...
            ParseStmntWithCommentOpt(ast_->global_stmnts_,
                                     std::bind(&GLSLParser::ParseGlobalStmnt,
                                               this));
        }
    ...
    SwitchCasePtr GLSLParser::ParseSwitchCase()
    {
        ...
        while (!Is(Tokens::Case) && !Is(Tokens::Default) && !Is(Tokens::RCurly))
            ParseStmntWithCommentOpt(ast_->stmnts_,
                                     std::bind(&GLSLParser::ParseStmnt, this));
    ...
    TypeSpecifierPtr GLSLParser::ParseTypeSpecifier(bool parse_void_type_a,
    ...
    VarDeclStmntPtr GLSLParser::ParseVarDeclStmnt(bool is_uniform_a,
                                                  const TokenPtr& ident_tkn_a)
    {
        (void)is_uniform_a;
        (void)ident_tkn_a;
    ...
    CallExprPtr GLSLParser::ParseCallExpr(const ObjectExprPtr& object_expr_a,
    ...
    TypeDenoterPtr GLSLParser::ParseTypeDenoter(bool allow_void_type_a,
    ...
    StructTypeDenoterPtr GLSLParser::ParseStructTypeDenoterWithStructDeclOpt(
            StructDeclPtr& struct_decl_a)
    {
        Accept(Tokens::Struct);
