...
namespace OL
{
    Parser::Parser(Log* log_a) : 
		report_handler_ {log_a}, log_ {log_a} {}
    ...
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseLogicAndExpr, this),
                                  {BinaryOp::LogicalOr});
    }

    ExprPtr Parser::ParseLogicAndExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseBitwiseOrExpr, this),
                                  {BinaryOp::LogicalAnd});
    }

    ExprPtr Parser::ParseBitwiseOrExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseBitwiseXOrExpr, this),
                                  {BinaryOp::Or});
    }

    ExprPtr Parser::ParseBitwiseXOrExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseBitwiseAndExpr, this),
                                  {BinaryOp::Xor});
    }

    ExprPtr Parser::ParseBitwiseAndExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseEqualityExpr, this),
                                  {BinaryOp::And});
    }

    ExprPtr Parser::ParseEqualityExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseRelationExpr, this),
    ...
    ExprPtr Parser::ParseRelationExpr()
    {
        if (ActiveParsingState().active_template_)
            return ParseLTRBinaryExpr(std::bind(&Parser::ParseShiftExpr, this),
                                      {BinaryOp::LessEqual,
    ...
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseAddExpr, this),
                                  {BinaryOp::LShift, BinaryOp::RShift});
    }

    ExprPtr Parser::ParseAddExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseSubExpr, this),
                                  {BinaryOp::Add});
    }

    ExprPtr Parser::ParseSubExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseMulExpr, this),
                                  {BinaryOp::Sub});
    }

    ExprPtr Parser::ParseMulExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseDivExpr, this),
                                  {BinaryOp::Mul});
    }

    ExprPtr Parser::ParseDivExpr()
    {
        return ParseLTRBinaryExpr(std::bind(&Parser::ParseValueExpr, this),
    ...
    {
        auto FindPrefix =
                [&ident_a](const std::string& prefix_) -> const std::string* {
    ...
    void Parser::AssertTokenSpell(const std::string& spell_a)
    {
        while (tkn_->Spell() != spell_a)
        {
            ...
            Error(R_UnexpectedTokenSpell(tkn_->Spell(), spell_a), true, false);
