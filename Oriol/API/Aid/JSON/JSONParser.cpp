...
namespace OL
{
    ...
    JSONParseException::JSONParseException(const std::string& message_a,
                                           size_t position_a)
    {
        message_ = "JSON parse error at position " + std::to_string(position_a)
                   + ": " + message_a;
    }
    ...
    void JSONObject::Add(const std::string& key_a,
                         std::unique_ptr<JSONValue> value_a)
    {
    ...
    std::unique_ptr<JSONObject> JSONParser::ParseObject(
            const std::string& input_a,
            size_t& index_a)
    {
    ...
    std::unique_ptr<JSONArray> JSONParser::ParseArray(
            const std::string& input_a,
            size_t& index_a)
    {
    ...
    std::string JSONParser::ParseString(const std::string& input_a,
                                        size_t& index_a)
    {
        std::string result_;
        ++index_a;
        ...
        while (index_a < input_a.size() && input_a[index_a] != '"')
        {
            ...
            else
            {
                ...
                {
                    ThrowParseError("Invalid control character in string.",
                                    index_a);
                }
    ...
    std::unique_ptr<JSONNumber> JSONParser::ParseNumber(
            const std::string& input_a,
            size_t& index_a)
    {
        ...
        if (index_a < input_a.size()
            && (input_a[index_a] == 'e' || input_a[index_a] == 'E'))
        {
            ++index_a;
            if (index_a < input_a.size()
                && (input_a[index_a] == '+' || input_a[index_a] == '-'))
                ++index_a;
    ...
    std::unique_ptr<JSONValue> JSONParser::ParseKeyword(
            const std::string& input_a,
            size_t& index_a)
    {
    ...
    std::unique_ptr<JSONValue> JSONParser::ParseFromFile(
            const std::string& path_to_file_a)
    {
        std::ifstream file_(path_to_file_a);
        if (!file_.is_open())
        {
            throw JSONParseException("Failed to open file: " + path_to_file_a,
                                     0);
        }
