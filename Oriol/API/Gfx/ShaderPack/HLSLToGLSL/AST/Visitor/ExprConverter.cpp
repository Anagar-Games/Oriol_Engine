...
namespace OL
{
            ...
            bool match_type_size_a)
    {
                    ...
                    target_type_den_a.As<BaseTypeDenoter>())
        {
            ...
            {
                return MustCastExprToDataType(base_target_type_den_->data_type_,
        ...
        if (IsVectorType(source_type_a) && IsVectorType(target_type_a))
        {
            ...
            if (source_dim_ < target_dim_)
            {
                ...
                for (auto i_ = source_dim_; i_ < target_dim_; ++i_)
                    args_.push_back(
                            ASTFactory::MakeLiteralExpr(base_data_type_, "0"));

                expr_a = ASTFactory::MakeTypeCtorCallExpr(type_denoter_, args_);
            } else
            {
    ...
    void ExprConverter::ConvertExprIfCastRequired(ExprPtr& expr_a,
                                                  const DataType target_type_a,
                                                  bool match_type_size_a)
    {
        if (expr_a)
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
        ...
        if (ast_a->expr_)
        {
            ...
            if (auto func_decl_ = ActiveFunctionDecl())
                ConvertExprTargetType(ast_a->expr_,
                                      func_decl_->return_type_->GetTypeDenoter()
                                              ->GetAliased());
        }
    }
    ...
    IMPLEMENT_VISIT_PROC(CallExpr)
    {
        ...
        {
            const auto& type_den_arg0_ =
                    ast_a->arguments_[0]->GetTypeDenoter()->GetAliased();

            const auto& type_den_arg1_ =
                    ast_a->arguments_[1]->GetTypeDenoter()->GetAliased();
    ...
    IMPLEMENT_VISIT_PROC(ArrayExpr)
    {
        ...
        for (auto& expr_ : ast_a->array_indices_)
        {
            ...
            if (auto base_type_den_ = type_den_.As<BaseTypeDenoter>())
            {
                ...
                ConvertExprTargetType(expr_, BaseTypeDenoter(int_vec_type_));
            }
        }
    }
...
#undef IMPLEMENT_VISIT_PROC
            ...
            ObjectExpr* object_expr_a)
    {
        if (!object_expr_a->symbol_ref_ && object_expr_a->prefix_expr_)
        {
            const auto& prefix_type_den_ =
                    object_expr_a->prefix_expr_->GetTypeDenoter()->GetAliased();
            ...
            if (prefix_type_den_.IsMatrix())
            {
                    ...
                    && !subscript_usage_.indices_.empty())
                {
                    ...
                    expr_a = ASTFactory::MakeArrayExpr(
                            object_expr_a->prefix_expr_,
                            {subscript_index_.first, subscript_index_.second});
                } else
                {
                    const auto wrapper_ident_ =
                            ExprConverter::GetMatrixSubscriptWrapperIdent(
                                    name_mangling_,
                                    subscript_usage_);
    ...
    void ExprConverter::ConvertExprVectorCompareUnary(ExprPtr& expr_a,
    ...
    void ExprConverter::ConvertExprImageAccessAssign(ExprPtr& expr_a,
    ...
    void ExprConverter::ConvertExprImageAccessArray(ExprPtr& expr_a,
            ...
            if (auto buffer_decl_ = buffer_type_den_->buffer_decl_ref_)
            {
                    ...
                    && num_dims_ < array_expr_a->NumIndices())
                {
                                        ...
                                        ->As<BaseTypeDenoter>())
                    {
                        ...
                        auto call_type_den_ = MakeBufferAccessCallTypeDenoter(
                        ...
                        if (assign_expr_a)
                        {
                            ...
                            } else
                            {
                                ...
                                const auto binary_op_ = AssignOpToBinaryOp(
                                        assign_expr_a->op_);

                                arg2_expr_ =
                                        ASTFactory::MakeBinaryExpr(lhs_expr_,
                                                                   binary_op_,
                                                                   rhs_expr_);
                            }
            ...
            ArrayExpr* array_expr_a)
    {
        ...
        std::size_t num_dims_ = 0;
        if (auto array_type_denoter_ = prefix_type_den_->As<ArrayTypeDenoter>())
        {
            num_dims_ = array_type_denoter_->array_dims_.size();
    ...
    void ExprConverter::ConvertExprIntrinsicCallLog10(ExprPtr& expr_a)
    {
        if (auto call_expr_ = expr_a->As<CallExpr>())
        {
                ...
                && call_expr_->arguments_.size() == 1)
            {
                ...
                const auto& type_den_ = arg0_->GetTypeDenoter()->GetAliased();
                ...
                if (auto base_type_den_ = type_den_.As<BaseTypeDenoter>())
                {
                    ...
                    auto literal_expr_ =
                            ASTFactory::MakeLiteralExpr(DataType::Int, "10");
                    literal_expr_->ConvertDataType(
                            BaseDataType(base_type_den_->data_type_));

                    auto rhs_expr_ =
                            ASTFactory::MakeIntrinsicCallExpr(Intrinsic::Log,
                                                              "log",
                                                              nullptr,
                                                              {literal_expr_});
                ...
                if (num_entries_ == init_expr_a->exprs_.size())
                {
                    ...
                    for (int row_ = 0; row_ < dims_.first; ++row_)
                    {
                        ...
                        auto row_init_ = ASTFactory::MakeInitializerExpr(
                                row_init_exprs_);
                        {
                            row_init_->GetTypeDenoter(row_type_denoter_.get());
                        }
    ...
    void ExprConverter::ConvertExprTextureBracketOp(ExprPtr& expr_a)
    {
        ...
        {
            if (auto array_expr_ = std::static_pointer_cast<ArrayExpr>(expr_a))
            {
                for (std::size_t i_ = 0; i_ + 1u < array_expr_->NumIndices();
                     ++i_)
                {
                ...
                auto type_den_ = array_expr_->prefix_expr_->GetTypeDenoter()
                                         ->GetSubArray(array_expr_->NumIndices()
                                                       - 1);
                if (auto buffer_type_den_ = type_den_->As<BufferTypeDenoter>())
                {
        ...
        if (auto call_expr_ = expr_a->As<CallExpr>())
        {
            ...
            if (intrinsic_ != Intrinsic::Undefined)
            {
                bool is_sample_intrinsic_ = IsTextureSampleIntrinsic(
                        intrinsic_);
                if (IsTextureLoadIntrinsic(intrinsic_) || is_sample_intrinsic_
                    || IsTextureGatherIntrisic(intrinsic_))
                {
                    ...
                    {
                        const auto& type_den_ =
                                call_expr_->GetTypeDenoter()->GetAliased();
                                    ...
                                    type_den_.As<BaseTypeDenoter>())
                        {
                            ...
                            {
                                const std::string vector_subscript_ = "rgb";
    ...
    void ExprConverter::ConvertExprCompatibleStruct(ExprPtr& expr_a)
    {
        if (auto object_expr_ = expr_a->As<ObjectExpr>())
        {
            if (object_expr_->prefix_expr_ && object_expr_->symbol_ref_)
            {
                ...
                {
                    const auto& prefix_type_den_ = object_expr_->prefix_expr_
                                                           ->GetTypeDenoter()
                                                           ->GetAliased();
                                    ...
                                    prefix_struct_type_den_->struct_decl_ref_)
                        {
                                        ...
                                        struct_decl_->compatible_struct_ref_)
                            {
                                ...
                                if (auto compat_var_decl_ =
                                            compat_struct_->IndexToMemberVar(
                                                    idx_))
                                {
