...
namespace OL
{
    ReflectionAnalyzer::ReflectionAnalyzer(Log* log_a) : report_handler_ {log_a}
    {}
    ...
    void ReflectionAnalyzer::Warning(const std::string& msg_a, const AST* ast_a)
    {
        if (enable_warnings_)
    ...
    int ReflectionAnalyzer::EvaluateConstExprInt(Expr& expr_a)
    {
        ...
        return static_cast<int>(
                expr_evaluator_.EvaluateOrDefault(expr_a, Variant::IntType(0))
                        .ToInt());
    }
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(SamplerDecl)
    {
        ...
        if (ast_a->sampler_values_.empty())
        {
            ...
            {
                sampler_state_.referenced_ = ast_a->flags_(AST::IsReachable);
                sampler_state_.type_ = SamplerTypeToResourceType(
                        ast_a->GetSamplerType());
                sampler_state_.name_ = ast_a->ident_;
                sampler_state_.slot_ = GetBindingPoint(ast_a->slot_registers_);
            }
    ...
    IMPLEMENT_VISIT_PROC(UniformBufferDecl)
    {
        ...
        {
            constant_buffer_.referenced_ = ast_a->flags_(AST::IsReachable);
            constant_buffer_.type_ = UniformBufferTypeToResourceType(
                    ast_a->buffer_type_);
            constant_buffer_.name_ = ast_a->ident_;
            constant_buffer_.slot_ = GetBindingPoint(ast_a->slot_registers_);
    ...
    IMPLEMENT_VISIT_PROC(BufferDeclStmnt)
    {
        ...
        for (auto& buffer_decl_ : ast_a->buffer_decls_)
        {
            ...
            {
                resource_.referenced_ = buffer_decl_->flags_(AST::IsReachable);
    ...
    IMPLEMENT_VISIT_PROC(VarDecl)
    {
        ...
        if (auto type_specifier_ = ast_a->FetchTypeSpecifier())
        {
            if (type_specifier_->is_uniform_)
            {
                ...
                {
                    attribute_.referenced_ = ast_a->flags_(AST::IsReachable);
                    attribute_.name_ = ast_a->ident_;
                    attribute_.slot_ = GetBindingPoint(ast_a->slot_registers_);
                }
...
#undef IMPLEMENT_VISIT_PROC
            ...
            Reflection::SamplerStateDesc& desc_a)
    {
        ...
        } else if (name_ == "BorderColor")
        {
                ...
                if (auto call_expr_ = ast_a->value_->As<CallExpr>())
                {
                        ...
                        && call_expr_->arguments_.size() == 4)
                    {
                        for (std::size_t i_ = 0; i_ < 4; ++i_)
                            desc_a.border_color_[i_] = EvaluateConstExprFloat(
            ...
            {
                Warning(R_FailedToInitializeSamplerValue(s_, "BorderColor"),
                        ast_a->value_.get());
            }
        }
    }
    ...
    void ReflectionAnalyzer::ReflectFieldType(Reflection::Field& field_a,
