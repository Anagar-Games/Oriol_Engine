...
namespace Oriol_Dev
{
    ...
    Value FPLParser::ParseValue()
    {
                   ...
                   || current_char_ == '+')
        {
            ...
            int dot_count_ = std::count(peek_ahead_.begin(),
                                        peek_ahead_.end(),
                                        '.');
            if (dot_count_ > 1)
            {
    ...
    void FPLParser::ParseKeyValuePair(
            std::map<std::string, Value>& current_map_a)
    {
    ...
    void FPLParser::ParseSection()
    {
        ...
        if (current_char_ != '{')
            throw std::runtime_error("Expected '{' after section name '"
                                     + current_section_ + "'");
        ...
        while (true)
        {
            ...
            {
                throw std::runtime_error("Unexpected end of file in section '"
                                         + current_section_ + "'");
            } else
            {
                throw std::runtime_error("Unexpected character in section '"
                                         + current_section_ + "': "
                                         + std::string(1, current_char_));
            }
        }
    ...
    std::map<std::string, std::map<std::string, Value>> FPLParser::Parse(
            std::istream& input_a)
    {
        input_ = &input_a;
        current_char_ = GetNextChar();
            ...
            while (current_char_ != EOF)
            {
                ...
                } else
                {
                    throw std::runtime_error(
                            "Unexpected character outside section: "
                            + std::string(1, current_char_));
                }
            }
