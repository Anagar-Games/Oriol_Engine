...
#define CALL_EXPR_FIND_PREDICATE(PREDICATE)                                    \
            ...
            const TypeDenoter* expected_type_denoter_a)
    {
        if (!buffered_type_denoter_ || expected_type_denoter_a)
            buffered_type_denoter_ = DeriveTypeDenoter(expected_type_denoter_a);
                     ...
                     unsigned int flags_a)
    {
        return const_cast<Expr*>(
                static_cast<const Expr*>(this)->Find(predicate_a, flags_a));
    }
    ...
    Expr* Expr::FindFirstNotOf(const Types expr_type_a, unsigned int flags_a)
    {
        return const_cast<Expr*>(
                static_cast<const Expr*>(this)->FindFirstNotOf(expr_type_a,
                                                               flags_a));
    }
            ...
            const std::vector<ExprPtr>& arguments_a)
    {
        IntrinsicUsage::ArgumentList arg_list_;
        {
            ...
            {
                const auto& type_den_ = arg_->GetTypeDenoter()->GetAliased();
    ...
    static const std::string* AttributeTypeToString(const AttributeType t_a)
    {
...
std::vector<int> ArrayDimension::GetArrayDimensionSizes(const std::vector<ArrayDimensionPtr>& array_dims_a)
{
            ...
            const std::initializer_list<StorageClass>& modifiers_a) const
    {
        for (auto mod_ : modifiers_a)
        {
            if (storage_classes_.find(mod_) != storage_classes_.end())
                return true;
        }
        return false;
    }
    ...
    TypeDenoterPtr VarDecl::DeriveTypeDenoter(const TypeDenoter*)
    {
        ...
        else
        {
            if (decl_stmnt_ref_)
            {
                return decl_stmnt_ref_->type_specifier_->type_denoter_->AsArray(
                        array_dims_);
            }
    ...
    {
        return (namespace_expr_ != nullptr ? static_member_var_ref_ : nullptr);
    }
    ...
    void VarDecl::SetCustomTypeDenoter(const TypeDenoterPtr& type_denoter_a)
    {
    ...
    {
        return std::make_shared<BufferTypeDenoter>(this)->AsArray(array_dims_);
    }
    ...
    {
        return std::make_shared<SamplerTypeDenoter>(this)->AsArray(array_dims_);
    }
                                      ...
                                      const StructDecl** owner_a) const
    {
        ...
        if (base_struct_ref_)
        {
            if (auto symbol_ = base_struct_ref_->FetchVarDecl(ident_a, owner_a))
                return symbol_;
        }
    ...
    StructDecl* StructDecl::FetchBaseStructDecl(const std::string& ident_a)
    {
            ...
            bool throw_error_if_no_match_a) const
    {
        ...
        {
            if (auto symbol_ =
                        base_struct_ref_->FetchFunctionDecl(ident_a,
                                                            arg_type_denoters_a,
                                                            owner_a))
                return symbol_;
        }
            ...
            bool include_base_structs_a) const
    {
        if (base_struct_ref_ && include_base_structs_a)
            base_struct_ref_->CollectMemberTypeDenoters(member_type_dens_a,
    ...
    void StructDecl::ForEachVarDecl(const VarDeclIteratorFunctor& iterator_a,
                                    bool include_base_structs_a)
    {
        if (base_struct_ref_ && include_base_structs_a)
            base_struct_ref_->ForEachVarDecl(iterator_a,
                                             include_base_structs_a);

        for (auto& member_ : var_members_)
        {
            const auto& type_den_ =
                    member_->type_specifier_->GetTypeDenoter()->GetAliased();
            if (auto struct_type_den_ = type_den_.As<StructTypeDenoter>())
            {
                if (auto struct_decl_ = struct_type_den_->struct_decl_ref_)
    ...
    std::size_t StructDecl::MemberVarToIndex(const VarDecl* var_decl_a,
                                             bool include_base_structs_a) const
    {
    ...
    static VarDecl* FindIndexOfStructMemberVar(const StructDecl& struct_decl_a,
                                               std::size_t& idx_a,
                                               bool include_base_structs_a)
    {
    ...
    VarDecl* StructDecl::IndexToMemberVar(std::size_t idx_a,
                                          bool include_base_structs_a) const
    {
        return FindIndexOfStructMemberVar(*this, idx_a, include_base_structs_a);
    }
    ...
    bool FunctionDecl::ParameterSemantics::Contains(VarDecl* var_decl_a) const
    {
                                       ...
                                       const Flags& compare_flags_a) const
    {
        ...
        for (std::size_t i_ = 0; i_ < parameters_.size(); ++i_)
        {
            ...
            const auto& rhs_type_den_ =
                    rhs_a.parameters_[i_]->var_decls_.front()->GetTypeDenoter();
    ...
    TypeDenoterPtr UniformBufferDecl::DeriveTypeDenoter(const TypeDenoter*)
    {
    ...
    bool UniformBufferDecl::AccumAlignedVectorSize(unsigned int& size_a,
                                                   unsigned int& padding_a,
                                                   unsigned int* offset_a)
    {
    ...
    void VarDeclStmnt::ForEachVarDecl(const VarDeclIteratorFunctor& iterator_a)
    {
        if (iterator_a)
        {
    ...
    const Expr* SequenceExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                   unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    TypeDenoterPtr TypeSpecifierExpr::DeriveTypeDenoter(const TypeDenoter*)
    {
    ...
    TypeDenoterPtr TernaryExpr::DeriveTypeDenoter(const TypeDenoter*)
    {
        ...
        const auto& then_type_den_ = then_expr_->GetTypeDenoter()->GetAliased();
        const auto& else_type_den_ = else_expr_->GetTypeDenoter()->GetAliased();
                            ...
                            common_type_den_->As<BaseTypeDenoter>())
                {
                    const auto sub_data_type_ =
                            VectorDataType(base_sub_type_den_->data_type_,
                                           cond_vec_size_);
                    return std::make_shared<BaseTypeDenoter>(sub_data_type_);
                }
            }
        }
    ...
    const Expr* TernaryExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                  unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    {
        const auto& cond_type_den_ = cond_expr_->GetTypeDenoter()->GetAliased();
    ...
    {
        const auto& lhs_type_den_ = lhs_expr_->GetTypeDenoter()->GetAliased();
        const auto& rhs_type_den_ = rhs_expr_->GetTypeDenoter()->GetAliased();
                                                           ...
                                                           this))
        {
            if (!lhs_type_den_.IsBase())
                RuntimeErr(R_OnlyBaseTypeAllowed(R_BinaryExpr(
    ...
    const Expr* BinaryExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                 unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    const Expr* UnaryExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    const Expr* CallExpr::Find(const FindPredicateConstFunctor& predicate_a,
                               unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    void CallExpr::ForEachOutputArgument(const ExprIteratorFunctor& iterator_a)
    {
        if (iterator_a)
        {
            if (auto func_decl_ = GetFunctionDecl())
            ...
            } else if (intrinsic_ != Intrinsic::Undefined)
            {
                const auto output_param_indices_ =
                        IntrinsicAdept::Get()
                                .GetIntrinsicOutputParameterIndices(intrinsic_);
        ...
        if (iterator_a)
        {
            ...
            } else if (intrinsic_ != Intrinsic::Undefined)
            {
                const auto param_type_denoters_ =
                        IntrinsicAdept::Get().GetIntrinsicParameterTypes(
                                intrinsic_,
                                arguments_);
                for (std::size_t i_ = 0,
                                 n_ = std::min(arguments_.size(),
    ...
    const Expr* BracketExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                  unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    const Expr* AssignExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                 unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    TypeDenoterPtr ObjectExpr::DeriveTypeDenoter(const TypeDenoter*)
    {
        ...
        {
            if (is_static_)
                RuntimeErr(R_IllegalStaticAccessForSubscript(ident_), this);
            else
    ...
    const Expr* ObjectExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                 unsigned int flags_a) const
    {
        if (predicate_a)
        {
            ...
            if (auto var_decl_ = symbol_ref_->As<VarDecl>())
            {
                if (prefix_expr_)
                {
                    if (max_tree_depth_a > 0)
                        return prefix_expr_->IsTrivialCopyable(max_tree_depth_a
                                                               - 1);
                } else
                    return true;
            }
                ...
                try
                {
                    auto vector_type_ =
                            SubscriptDataType(base_type_den_->data_type_,
                                              ident_);
                    return std::make_shared<BaseTypeDenoter>(vector_type_);
                } catch (const std::exception& e_)
                {
                    RuntimeErr(e_.what(), this);
                }
    ...
    const Expr* ArrayExpr::Find(const FindPredicateConstFunctor& predicate_a,
                                unsigned int flags_a) const
    {
        if (predicate_a)
        {
    ...
    const Expr* CastExpr::Find(const FindPredicateConstFunctor& predicate_a,
                               unsigned int flags_a) const
    {
        if (predicate_a)
        {
            ...
            const TypeDenoter* expected_type_denoter_a)
    {
        ...
        if (auto base_type_den_ = type_den_.As<BaseTypeDenoter>())
        {
            ...
            if (IsScalarType(data_type_))
            {
                ...
                if (!expr0_type_den_->IsCastableTo(*expected_type_denoter_a))
                    RuntimeErr(
                            R_IllegalCast(expr0_type_den_->ToString(),
                                          expected_type_denoter_a->ToString(),
                                          R_InitializerList),
                            this);
            }
        } else if (auto array_type_den_ = type_den_.As<ArrayTypeDenoter>())
        {
            for (auto& expr_ : exprs_)
                expr_->GetTypeDenoter(array_type_den_->sub_type_denoter_.get());
        }
            ...
            std::size_t layer_a)
    {
        if (layer_a < array_indices_a.size())
        {
                ...
                && static_cast<std::size_t>(idx_) < ast_a->exprs_.size())
            {
                ...
                if (auto sub_init_expr_ = expr_->As<const InitializerExpr>())
                    return FetchSubExprFromInitializerExpr(sub_init_expr_,
                                                           array_indices_a,
                                                           layer_a + 1);

                RuntimeErr(R_ExpectedInitializerForArrayAccess, expr_.get());
            }
            ...
            std::size_t layer_a)
    {
        if (layer_a < array_indices_a.size())
        {
                ...
                && static_cast<std::size_t>(idx_) < ast_a->exprs_.size())
            {
                ...
                if (auto sub_init_expr_ = expr_->As<const InitializerExpr>())
                {
                    if (NextArrayIndicesFromInitializerExpr(sub_init_expr_,
                                                            array_indices_a,
                                                            layer_a + 1))
                        return true;
                }
    ...
    {
        return NextArrayIndicesFromInitializerExpr(this, array_indices_a, 0);
    }
} // namespace OL
