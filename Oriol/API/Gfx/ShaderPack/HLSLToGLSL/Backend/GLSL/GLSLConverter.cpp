...
namespace OL
{
            ...
            const DataType data_type_a)
    {
        if (auto var_decl_stmnt_ = var_decl_a.decl_stmnt_ref_)
        {
                                             ...
                                             .As<BaseTypeDenoter>())
            {
                ...
                {
                    auto new_var_type_den_ = std::make_shared<BaseTypeDenoter>(
                            data_type_a);
    ...
    void GLSLConverter::ConvertASTPrimary(Program& program_a,
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(Program)
    {
        ...
        RegisterGlobalDeclIdents(entry_point_->input_semantics_.var_decl_refs_);
    ...
    IMPLEMENT_VISIT_PROC(CallExpr)
    {
        ...
        {
            if (IsTextureIntrinsic(ast_a->intrinsic_) && ast_a->prefix_expr_)
            {
                    ...
                    && ast_a->arguments_.size() >= 2)
                {
                    ...
                    if (IsSamplerStateTypeDenoter(arg1_expr_->GetTypeDenoter()))
                    {
                        ast_a->MergeArguments(
                                0,
                                ASTFactory::MakeTextureSamplerBindingCallExpr);
                    }
                }
            }
        }
    ...
    IMPLEMENT_VISIT_PROC(VarDecl)
    {
        if (ast_a->IsStatic())
        {
            ...
            {
                ast_a->ident_ = struct_decl_->ident_ + "_" + ast_a->ident_;

                ast_a->ident_.AppendPrefix(GetNameMangling().namespace_prefix_);
            }
        }
    ...
    IMPLEMENT_VISIT_PROC(StructDecl)
    {
        ...
        {
            auto base_member_type_den_ = std::make_shared<StructTypeDenoter>(
                    base_struct_);
    ...
    IMPLEMENT_VISIT_PROC(VarDeclStmnt)
    {
        ...
        {
            ast_a->type_specifier_->type_modifiers_.erase(TypeModifier::Const);
        }
        ...
        if (auto struct_type_den_ = type_den_.As<StructTypeDenoter>())
        {
            if (auto struct_decl_ = struct_type_den_->struct_decl_ref_)
            {
                ...
                if (ast_a->expr_->FindFirstOf(AST::Types::CallExpr))
                {
                    ...
                    auto temp_var_decl_stmnt_ = ASTFactory::MakeVarDeclStmnt(
...
#undef IMPLEMENT_VISIT_PROC
            ...
            std::vector<VarDeclStmntPtr>& stmnts_a)
    {
        MoveAllIf(
                ...
                [&](const VarDeclStmntPtr& var_decl_stmnt_) {
                    return IsSamplerStateTypeDenoter(
                            var_decl_stmnt_->type_specifier_->GetTypeDenoter());
                });
    }
    ...
    bool GLSLConverter::RenameReservedKeyword(Identifier& ident_a)
    {
        ...
        } else
        {
            ...
            {
                ident_a.AppendPrefix(GetNameMangling().reserved_word_prefix_);
                return true;
            }
    ...
    void GLSLConverter::ConvertFunctionDecl(FunctionDecl* ast_a)
    {
        ...
        if (auto struct_decl_ = ast_a->struct_decl_ref_)
        {
            if (!ast_a->IsStatic())
            {
                auto self_param_type_den_ = std::make_shared<StructTypeDenoter>(
                        struct_decl_);
                                        ...
                                        const Flags& flags_a)
    {
                    ...
                    type_den_a.GetAliased().As<StructTypeDenoter>())
        {
            if (auto struct_decl_ = struct_type_den_->struct_decl_ref_)
            {
                struct_decl_->ForEachVarDecl([&flags_a](VarDeclPtr& member_) {
            ...
            if (!param_->var_decls_.empty())
            {
                ...
                if (IsTypeDynamicArrayInGLSL(type_den_))
                {
                    ...
                    if (auto sub_type_den_ = type_den_.FetchSubTypeDenoter())
                        AddFlagsToStructMembers(*sub_type_den_,
    ...
    void GLSLConverter::ConvertIntrinsicCallSaturate(CallExpr* ast_a)
    {
        if (ast_a->arguments_.size() == 1)
        {
            ...
            RuntimeErr(R_InvalidIntrinsicArgCount(ast_a->ident_,
                                                  1,
    ...
    void GLSLConverter::ConvertIntrinsicCallTextureLOD(CallExpr* ast_a)
    {
        if (ast_a->arguments_.size() == 2)
        {
            ...
            if (auto texture_dim_ = ExprConverter::GetTextureDimFromExpr(
                        args_[0].get()))
            {
                ExprConverter::ConvertExprIfCastRequired(args_[1],
                                                         DataType::Float4,
                                                         true);
                ...
                auto arg2_expr_ = ASTFactory::MakeObjectExpr(sub_expr_, "w");
    ...
    void GLSLConverter::ConvertIntrinsicCallTextureSampleLevel(CallExpr* ast_a)
    {
        if (auto texture_dim_ = GetTextureDimFromIntrinsicCall(ast_a))
        {
            auto& args_ = ast_a->arguments_;
    ...
    void GLSLConverter::ConvertIntrinsicCallTextureLoad(CallExpr* ast_a)
    {
        if (auto texture_dim_ = GetTextureDimFromIntrinsicCall(ast_a))
        {
            ...
            const auto& type_den_ = args_[0]->GetTypeDenoter()->GetAliased();
            if (auto buffer_type_den_ = type_den_.As<BufferTypeDenoter>())
            {
                               ...
                               == BufferType::Texture2DMSArray)
                {
                    if (ast_a->intrinsic_ == Intrinsic::Texture_Load_3)
                        RuntimeErr(R_FailedToMapClassIntrinsicOverload(
                                           ast_a->ident_,
                                           BufferTypeToString(
                                                   buffer_type_den_
                                                           ->buffer_type_)),
                    ...
                    if (args_.size() >= 3)
                        ExprConverter::ConvertExprIfCastRequired(args_[2],
                                                                 DataType::Int,
                                                                 true);
                } else
                {
                    ...
                    {
                        auto idx_arg_expr_ =
                                ASTFactory::MakeLiteralExpr(DataType::Int, "0");
        ...
        if (args_.size() >= 2)
        {
            ...
            if (auto arg0_array_expr_ = arg0_expr_->As<ArrayExpr>())
            {
                            ...
                            prefix_type_den_->As<BufferTypeDenoter>())
                {
                    ...
                    {
                        ast_a->intrinsic_ = InterlockedToImageAtomicIntrinsic(
                                ast_a->intrinsic_);
            ...
            if (IsRWImageBufferType(buffer_type_))
            {
                const auto num_dims_ = GetBufferTypeTextureDim(buffer_type_);
    ...
    void GLSLConverter::ConvertIntrinsicCallGather(CallExpr* ast_a)
    {
        ...
        if (offset_count_ == 4)
        {
            ...
            auto array_ctor_expr_ =
                    ASTFactory::MakeTypeCtorCallExpr(array_type_denoter_,
                                                     array_ctor_arguments_);

            ast_a->arguments_.erase(ast_a->arguments_.begin()
                                            + offset_arg_start_,
                                    ast_a->arguments_.begin()
                                            + offset_arg_end_);
    ...
    void GLSLConverter::ConvertIntrinsicCallSampleCmp(CallExpr* ast_a)
    {
        ...
        if (auto texture_dim_ = GetTextureDimFromIntrinsicCall(ast_a))
        {
            ...
            if (args_.size() >= 3)
            {
                ...
                if (texture_dim_ < 4)
                {
                    DataType target_type_ = VectorDataType(DataType::Float,
                                                           texture_dim_ + 1);
                    auto type_denoter_ = std::make_shared<BaseTypeDenoter>(
                            target_type_);

                    args_[1] = ASTFactory::MakeTypeCtorCallExpr(type_denoter_,
                                                                {args_[1],
                                                                 args_[2]});
    ...
    void GLSLConverter::ConvertEntryPointStructPrefix(ExprPtr& expr_a,
        ...
        if (auto var_decl_ = prefix_expr_a->FetchVarDecl())
        {
            ...
            if (auto struct_type_den_ = var_type_den_.As<StructTypeDenoter>())
            {
                if (!MakeObjectExprImmutableForNEPStruct(
                            ...
                            struct_type_den_->struct_decl_ref_))
                {
                    if (IsGlobalInOutVarDecl(object_expr_a->FetchVarDecl()))
                        expr_a.reset();
                }
            }
        }
        ...
        struct_decl_a->ForEachVarDecl([&](VarDeclPtr& var_decl_) {
            if (idx_ < type_constructor_a.exprs_.size())
            {
                auto assign_stmnt_ = ASTFactory::MakeAssignStmnt(
                        ASTFactory::MakeObjectExpr(prefix_expr_,
    ...
    void GLSLConverter::ConvertObjectExprStaticVar(ObjectExpr* object_expr_a)
    {
        ...
        if (owner_struct_decl_a && caller_struct_decl_a)
        {
            if (owner_struct_decl_a->IsBaseOf(caller_struct_decl_a, true))
            {
                if (use_self_param_a)
                {
                    ...
                    {
                        prefix_expr_a = ASTFactory::MakeObjectExpr(self_param_);
                    }
                }
            }
    ...
    void GLSLConverter::ConvertObjectPrefixSelfParam(ExprPtr& prefix_expr_a,
                                                     ObjectExpr* object_expr_a)
    {
    ...
    void GLSLConverter::ConvertObjectPrefixBaseStruct(ExprPtr& prefix_expr_a,
                                                      ObjectExpr* object_expr_a)
    {
                    ...
                    prefix_type_den_.As<StructTypeDenoter>())
        {
                        ...
                        prefix_struct_type_den_->struct_decl_ref_)
            {
                if (auto var_decl_ = object_expr_a->FetchVarDecl())
                {
                    ConvertObjectPrefixStructMember(prefix_expr_a,
    ...
    void GLSLConverter::ConvertObjectPrefixNamespace(ExprPtr& prefix_expr_a,
                                                     ObjectExpr* object_expr_a)
    {
        if (auto prefix_object_expr_ = prefix_expr_a->As<ObjectExpr>())
        {
                        ...
                        prefix_object_expr_->FetchSymbol<StructDecl>())
            {
                ...
                {
                    const auto& prefix_type_den_ =
                            prefix_object_expr_->prefix_expr_->GetTypeDenoter()
                                    ->GetAliased();
                ...
                {
                    if (auto active_struct_decl_ = ActiveStructDecl())
                        ConvertObjectPrefixNamespaceStruct(prefix_object_expr_,
                                                           object_expr_a,
                                                           base_struct_decl_,
                                                           active_struct_decl_);
                }
            ...
            const StructDecl* caller_struct_decl_a)
    {
        if (owner_struct_decl_a->IsBaseOf(caller_struct_decl_a))
        {
            ...
            {
                if (auto base_member_ = caller_struct_decl_a->FetchBaseMember())
        ...
        if (auto array_type_den_ = type_den_.As<ArrayTypeDenoter>())
        {
                        ...
                        var_decl_a->initializer_->As<InitializerExpr>())
            {
                ...
                do
                {
                    ...
                    auto assign_stmnt_ =
                            ASTFactory::MakeArrayAssignStmnt(var_decl_a,
                                                             array_indices_,
                                                             sub_expr_);
