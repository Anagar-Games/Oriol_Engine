...
namespace OL
{
    ...
    void Analyzer::SubmitReport(bool is_error_a,
    ...
    void Analyzer::ErrorUndeclaredIdent(const std::string& ident_a,
                                        ...
                                        const AST* ast_a)
    {
        Error(R_UndeclaredIdent(ident_a, context_name_a, similar_ident_a),
              ast_a);
    }
    ...
    AST* Analyzer::FetchFromCurrentScopeOrNull(const std::string& ident_a) const
    {
    ...
    StructDecl* Analyzer::FetchStructDeclFromIdent(const std::string& ident_a,
                                                   const AST* ast_a)
    {
    ...
    StructDecl* Analyzer::FindCompatibleStructDecl(const StructDecl& rhs_a)
    {
                                      ...
                                      const AST* ast_a)
    {
        if (type_denoter_a)
        {
            if (auto buffer_type_den_ = type_denoter_a->As<BufferTypeDenoter>())
        ...
        if (auto alias_type_den_ = type_denoter_a->As<AliasTypeDenoter>())
        {
            if (!alias_type_den_->alias_decl_ref_)
            {
                if (auto symbol_ = FetchType(alias_type_den_->ident_, ast_a))
                {
                    ...
                    {
                        type_denoter_a = std::make_shared<StructTypeDenoter>(
                                struct_decl_);
                                        ...
                                        const std::string& context_desc_a)
    {
        if (auto dest_type_den_ = GetTypeDenoterFrom(dest_ast_a))
        {
            if (auto source_type_den_ =
                        GetTypeDenoterFrom(source_ast_a, dest_type_den_.get()))
            {
    ...
    void Analyzer::AnalyzeConditionalExpression(Expr* expr_a)
    {
        if (expr_a)
        {
            ...
            const auto& cond_type_den_ = expr_a->GetTypeDenoter()->GetAliased();
            if (!cond_type_den_.IsScalar())
                Error(R_ConditionalExprNotScalar(cond_type_den_.ToString()),
                      expr_a);
        }
    }
    ...
    Variant Analyzer::EvaluateConstExpr(Expr& expr_a)
    {
        try
        {
            ...
            return expr_evaluator_
                    .Evaluate(expr_a, [this](ObjectExpr* expr_) -> Variant {
    ...
    std::string Analyzer::FetchSimilarIdent(const std::string& ident_a,
    ...
    {
        if (WarnEnabled(Warnings::UnusedVariables) && !InsideGlobalScope()
            && symbol_a)
        {
