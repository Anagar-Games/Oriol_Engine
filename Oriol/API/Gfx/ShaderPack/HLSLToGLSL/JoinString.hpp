...
#ifndef OL_JOINSTRING_HPP
#define OL_JOINSTRING_HPP
...
namespace OL
{
    ...
    template <typename T>
    void ToStringListPrimary(std::vector<std::string>& list_a, const T& value_a)
    {
        list_a.push_back(value_a);
    }

    template <>
    void ToStringListPrimary<std::size_t>(std::vector<std::string>& list_a,
    ...
    void ToStringListSecondary(std::vector<std::string>& list_a, Arg0&& arg0_a)
    {
        ToStringListPrimary(list_a, std::forward<Arg0>(arg0_a));
    }
    ...
    public:
        ...
        std::string Join(const std::vector<std::string>& values_a = {}) const;
