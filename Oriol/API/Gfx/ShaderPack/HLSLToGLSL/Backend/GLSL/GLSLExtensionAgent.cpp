...
namespace OL
{
    ...
    void GLSLExtensionAgent::EstablishIntrinsicExtMap()
    {
        ...
        if (target_glsl_version_ == OutputShaderVersion::ESSL100)
        {
            ...
            intrinsic_ext_map_[Intrinsic::DDX] = E_GL_OES_standard_derivatives;
            intrinsic_ext_map_[Intrinsic::DDY] = E_GL_OES_standard_derivatives;
        }
    }

    void GLSLExtensionAgent::AcquireExtension(const std::string& extension_a,
                                              ...
                                              const AST* ast_a)
    {
        ...
        {
            const auto required_version_ = static_cast<OutputShaderVersion>(
                    it_->second);
                       ...
                       < GetGLSLVersionNumber(required_version_))
            {
                if (allow_extensions_)
                {
                    ...
                    if (on_report_extension_)
                        on_report_extension_(R_GLSLExtensionAcquired(
                                                     extension_a,
                                                     ToString(
                                                             required_version_),
                                                     reason_a),
                ...
                {
                    if (on_report_extension_)
                        on_report_extension_(R_GLSLExtensionOrVersionRequired(
                                                     extension_a,
                                                     ToString(
                                                             required_version_),
                                                     reason_a),
            ...
            RuntimeErr(R_NoGLSLExtensionVersionRegisterd(extension_a), ast_a);
    }
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(VarDecl)
    {
        if (ast_a->GetTypeDenoter()->NumDimensions() >= 2)
            AcquireExtension(E_GL_ARB_arrays_of_arrays, R_MultiDimArray, ast_a);
        ...
        if (ast_a->flags_(AST::IsReachable))
        {
            ...
            if (IsRWBufferType(buffer_type_))
            {
                if (buffer_type_ == BufferType::RWStructuredBuffer
                    ...
                    || buffer_type_ == BufferType::ConsumeStructuredBuffer)
                {
                    AcquireExtension(E_GL_ARB_shader_storage_buffer_object,
    ...
    IMPLEMENT_VISIT_PROC(BinaryExpr)
    {
        if (IsBitwiseOp(ast_a->op_) || ast_a->op_ == BinaryOp::Mod)
            AcquireExtension(E_GL_EXT_gpu_shader4, R_BitwiseOperator, ast_a);
