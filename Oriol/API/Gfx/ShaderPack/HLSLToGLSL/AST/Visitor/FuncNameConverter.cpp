                ...
                if (auto func_lhs_ = func_list_a[i_])
                {
                    for (std::size_t j_ = i_ + 1; j_ < n_; ++j_)
                    {
                        if (auto func_rhs_ = func_list_a[j_])
                        {
                            ...
                            {
                                if (name_index_ == 0)
                                    ConvertMemberFunctionName(*func_lhs_,
                                                              name_index_);
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(FunctionDecl)
    {
        ...
        if (conversion_flags_(RenameMemberFunctions))
        {
            ...
            {
                ast_a->ident_.RemovePrefix(name_mangling_.namespace_prefix_);

                ast_a->ident_ = struct_decl_->ident_ + "_" + ast_a->ident_;

                ast_a->ident_.AppendPrefix(name_mangling_.namespace_prefix_);
            }
        }
