...
namespace OL
{
                                      ...
                                      ShaderTarget target_a)
    {
        ...
        auto input_stream_ = std::make_shared<std::ifstream>(hlsl_path_a,
                                                             std::ios::binary);
        ...
        std::vector<uint8_t> glsl_binary_ = CompileGLSL(glsl_source_, target_a);
    ...
    std::vector<uint8_t> ShaderPack::CompileHLSL(const std::string& source_a,
                                                 ...
                                                 ShaderTarget target_a)
    {
        ...
        if (FAILED(hr_))
        {
            if (error_)
            {
                std::string msg_(static_cast<char*>(error_->GetBufferPointer()),
                                 error_->GetBufferSize());
                error_->Release();
                throw std::runtime_error("HLSL compilation failed: " + msg_);
            }
    ...
    std::vector<uint8_t> ShaderPack::CompileGLSL(const std::string& source_a,
                                                 ShaderTarget target_a)
    {
        ...
        if (compile_status_ != GL_TRUE)
        {
            ...
            throw std::runtime_error("GLSL compilation failed: " + info_log_);
        }
        ...
        if (link_status_ != GL_TRUE)
        {
            ...
            std::string info_log_(log_length_, '\0');
            glGetProgramInfoLog(program_, log_length_, nullptr, &info_log_[0]);
    ...
    void ShaderPack::WriteShaderPack(const std::string& path_a,
            ...
            const char* section_tag_a)
    {
        ...
        if (!file_)
            throw std::runtime_error("Cannot open shader pack file: " + path_a);
