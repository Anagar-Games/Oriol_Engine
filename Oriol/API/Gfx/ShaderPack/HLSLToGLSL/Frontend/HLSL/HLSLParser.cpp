...
namespace OL
{
    ...
    ProgramPtr HLSLParser::ParseSource(const SourceCodePtr& source_a,
    ...
    {
        return std::make_shared<HLSLScanner>(enable_cg_keywords_, GetLog());
    }

    bool HLSLParser::IsDataType() const
    {
        return (IsBaseDataType() || Is(Tokens::Vector) || Is(Tokens::Matrix)
                || Is(Tokens::Buffer) || Is(Tokens::Sampler)
                || Is(Tokens::SamplerState));
    }
    ...
    bool HLSLParser::IsModifier() const
    {
        return (Is(Tokens::InputModifier) || Is(Tokens::InterpModifier)
                || Is(Tokens::TypeModifier) || Is(Tokens::StorageClass));
    }
    ...
    bool HLSLParser::IsRegisteredTypeName(const std::string& ident_a) const
    {
    ...
    void HLSLParser::GeneratePreDefinedTypeAliases(Program& ast_a)
    {
        ...
        for (const auto& type_ : pre_defined_types_)
        {
            ast_a.global_stmnts_.push_back(
                    MakeAndRegisterBuildinAlias(type_.first, type_.second));
        }
    }
    ...
    ProgramPtr HLSLParser::ParseProgram(const SourceCodePtr& source_a)
    {
        ...
        while (true)
        {
            ...
            ParseStmntWithCommentOpt(ast_->global_stmnts_,
                                     std::bind(&HLSLParser::ParseGlobalStmnt,
                                               this));
        }
    ...
    RegisterPtr HLSLParser::ParseRegister(bool parse_colon_a)
    {
        ...
        if (type_ident_.size() > 1)
            ast_->slot_ = ParseIntLiteral(type_ident_.substr(1),
    ...
    BufferDeclPtr HLSLParser::ParseBufferDecl(BufferDeclStmnt* decl_stmnt_ref_a,
                                              const TokenPtr& ident_tkn_a)
    {
            ...
            for (auto& stmnt_ : ast_->local_stmnts_)
            {
                if (stmnt_->Type() == AST::Types::VarDeclStmnt)
                {
                    ast_->var_members_.push_back(
                            std::static_pointer_cast<VarDeclStmnt>(stmnt_));
                                   ...
                                   stmnt_->As<BasicDeclStmnt>())
                {
                        ...
                        == AST::Types::FunctionDecl)
                    {
                        ...
                        Error(R_IllegalDeclStmntInsideDeclOf(ast_->ToString()),
                              stmnt_->area_,
                              false);
                } else
                    Error(R_IllegalDeclStmntInsideDeclOf(ast_->ToString()),
    ...
    StmntPtr HLSLParser::ParseStmntWithIdent()
    {
        ...
        if (Is(Tokens::LBracket) || Is(Tokens::UnaryOp) || Is(Tokens::BinaryOp)
            || Is(Tokens::TernaryOp))
        {
        ...
        } else if (expr_ == object_expr_)
        {
            ...
            ast_->type_specifier_->type_denoter_ = ParseTypeDenoterWithArrayOpt(
    ...
    CallExprPtr HLSLParser::ParseCallExpr(const ObjectExprPtr& object_expr_a,
    ...
    std::vector<StmntPtr> HLSLParser::ParseGlobalStmntList()
    {
        ...
        {
            ParseAndIgnoreTechniquesAndNullStmnts();
            ParseStmntWithCommentOpt(stmnts_,
                                     std::bind(&HLSLParser::ParseGlobalStmnt,
                                               this));
        }
    ...
    TypeDenoterPtr HLSLParser::ParseTypeDenoter(bool allow_void_type_a,
            ...
            PushParsingState({true});
            {
                ...
                if (Is(Tokens::TypeModifier))
                {
                    ...
                    if (modifier_ != TypeModifier::SNorm
                        ...
                        Error(R_InvalidModifierForGenericTypeDen(modifier_str_),
                              true,
                              false);
                }

                type_denoter_->generic_type_denoter_ = ParseTypeDenoter(false);
                ...
                if (Is(Tokens::Comma))
                {
                    ...
                    if (IsTextureMSBufferType(type_denoter_->buffer_type_))
                    {
                        if (gen_size_ < 1 || gen_size_ >= 128)
                            Warning(R_TextureSampleCountLimitIs128(gen_size_),
                                    buffer_type_tkn_.get());
                    } else if (IsPatchBufferType(type_denoter_->buffer_type_))
                    {
    ...
    StructTypeDenoterPtr HLSLParser::ParseStructTypeDenoterWithStructDeclOpt(
            StructDeclPtr& struct_decl_a)
    {
        ...
        if (Is(Tokens::LCurly))
        {
            ...
            return std::make_shared<StructTypeDenoter>(struct_decl_a.get());
        } else
        {
    ...
    AliasTypeDenoterPtr HLSLParser::ParseAliasTypeDenoter(std::string ident_a)
    {
        if (ident_a.empty())
            ident_a = ParseIdent();
            ...
            } else if (Is(Tokens::PackOffset))
            {
                ...
                if (allow_pack_offset_a)
                {
                    if (var_decl_a.pack_offset_)
                        Warning(R_PackOffsetOverridden, pack_offset_->area_);
    ...
    IndexedSemantic HLSLParser::ParseSemantic(bool parse_colon_a)
    {
        try
        {
            if (parse_colon_a)
                Accept(Tokens::Colon);
            return HLSLKeywordToSemantic(ParseIdent(), use_d3d10_semantics_);
        } catch (const std::exception& e_)
        {
            Error(e_.what());
        }
                                    ...
                                    bool allow_primitive_type_a)
    {
        ...
        {
            type_specifier_a->interp_modifiers_.insert(ParseInterpModifier());
        } else if (Is(Tokens::TypeModifier))
        {
            type_specifier_a->SetTypeModifier(ParseTypeModifier());
        } else if (Is(Tokens::StorageClass))
        {
            type_specifier_a->storage_classes_.insert(ParseStorageClass());
        } else if (Is(Tokens::PrimitiveType))
        {
            ...
            if (type_specifier_a->primitive_type_ == PrimitiveType::Undefined)
