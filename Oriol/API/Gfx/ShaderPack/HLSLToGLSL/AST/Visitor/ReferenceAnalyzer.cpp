...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
        ...
        if (Reachable(ast_a))
        {
            if (ast_a->IsForwardDecl())
            {
                if (ast_a->func_impl_ref_)
                    ...
                    RuntimeErr(R_MissingFuncImpl(ast_a->ToString(false)),
                               ast_a);
            } else
            {
                for (auto func_forward_decl_ : ast_a->func_forward_decl_refs_)
    ...
    IMPLEMENT_VISIT_PROC(BufferDeclStmnt)
    {
        if (Reachable(ast_a))
        {
            if (auto generic_type_denoter_ =
                        ast_a->type_denoter_->generic_type_denoter_.get())
            {
    ...
    IMPLEMENT_VISIT_PROC(CallExpr)
    {
        ...
        {
            auto func_call_it_ = std::find_if(
                    call_expr_stack_.begin(),
                    call_expr_stack_.end(),
                    [func_decl_](CallExpr* call_expr_) {
                        return (call_expr_->GetFunctionImpl() == func_decl_);
                    });
            ...
            if (func_call_it_ != call_expr_stack_.end())
            {
                ...
                for (auto func_call_ : call_expr_stack_)
                    ReportHandler::HintForNextReport(
                            "  '"
                            + func_call_->GetFunctionDecl()->ToString(false)
        ...
        if (ast_a->intrinsic_ != Intrinsic::Undefined)
        {
                ...
                || ast_a->intrinsic_ == Intrinsic::Image_Load)
            {
                if (!ast_a->arguments_.empty())
                {
                    ...
                    {
                        if (IsRWImageBufferType(buffer_type_den_->buffer_type_))
                        {
        ...
        ast_a->ForEachOutputArgument([this](ExprPtr& arg_expr_, VarDecl*) {
        ...
        if (ast_a->prefix_expr_)
        {
            ...
            if (prefix_type_den_.IsMatrix())
            {
                ...
                program_->used_matrix_subscripts_.insert(
                        {prefix_base_type_den_->data_type_, ast_a->ident_});
            }
        }
