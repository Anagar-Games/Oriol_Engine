...
namespace OL
{
    ...
    TokenPtr Scanner::Make(const Token::Types& type_a, bool take_chr_a)
    {
        if (take_chr_a)
        {
            std::string spell_;
            spell_ += TakeIt();
            return std::make_shared<Token>(Pos(), type_a, std::move(spell_));
        }
    ...
    void Scanner::Error(const std::string& msg_a)
    {
        throw Report(ReportTypes::Error,
                     R_LexicalError + " (" + Pos().ToString() + ") : " + msg_a);
    }
    ...
    {
        while (std::isspace(UChr()) && (include_new_lines_a || !IsNewLine()))
            TakeIt();
    }
    ...
    TokenPtr Scanner::ScanNumber(bool start_with_period_a)
    {
        ...
        } else
        {
            ...
            {
                spell_ += TakeIt();
                while (std::isdigit(UChr()) || (Chr() >= 'a' && Chr() <= 'f')
                       || (Chr() >= 'A' && Chr() <= 'F'))
                    spell_ += TakeIt();
            }
    ...
    void Scanner::AppendComment(const std::string& s_a)
    {
        ...
        if (comment_start_pos_ > 0)
        {
            ...
            if (first_not_ == std::string::npos)
                ...
                comment_ += s_a.substr(std::min(
                        first_not_,
                        static_cast<std::size_t>(comment_start_pos_ - 1)));
        } else
        {
            comment_ += s_a;
        }
