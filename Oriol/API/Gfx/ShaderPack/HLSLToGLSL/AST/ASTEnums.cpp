...
{
    [[noreturn]]
    static void MapFailed(const std::string& from_a, const std::string& to_a)
    {
    ...
    MatrixSubscriptUsage::MatrixSubscriptUsage(const DataType data_type_in_a,
    ...
    std::string DataTypeToString(const DataType t_a, bool use_template_syntax_a)
    {
    ...
    bool IsRealType(const DataType t_a)
    {
        return ((t_a >= DataType::Half && t_a <= DataType::Double)
                ...
                || (t_a >= DataType::Half2x2 && t_a <= DataType::Double4x4));
    }
    ...
    bool IsSingleRealType(const DataType t_a)
    {
        return ((t_a == DataType::Float)
                ...
                || (t_a >= DataType::Float2x2 && t_a <= DataType::Float4x4));
    }
    ...
    bool IsDoubleRealType(const DataType t_a)
    {
        return ((t_a == DataType::Double)
                ...
                || (t_a >= DataType::Double2x2 && t_a <= DataType::Double4x4));
    }
...
#undef FIND_BASETYPE
        ...
        auto IsValidSubscript = [&](std::string compare_subscript_,
            ...
            for (auto chr_ : subscript_a)
            {
                ...
                if (pos_ != std::string::npos)
                    subscript_indices_.push_back({static_cast<int>(pos_), 0});
                else
                    return false;
            }
            ...
            std::vector<std::pair<int, int>>* indices_a)
    {
        ...
        if (s_a[i_a] != '_')
            InvalidArg(R_InvalidCharInMatrixSubscript(std::string(1, s_a[i_a]),
                                                      s_a));
        ++i_a;
    ...
    DataType SubscriptDataType(const DataType data_type_a,
                               const std::string& subscript_a,
                               std::vector<std::pair<int, int>>* indices_a)
    {
    ...
    {
        return (t_a >= BufferType::RWBuffer && t_a <= BufferType::RWTexture3D);
    }
    ...
    DataType GetImageLayoutFormatBaseType(const ImageLayoutFormat format_a)
    {
    ...
    {
        return (t_a >= AttributeType::Align && t_a <= AttributeType::HfbStride);
    }
    ...
    {
        return (t_a >= Intrinsic::Image_Load && t_a <= Intrinsic::Image_Store);
    }
    ...
    IndexedSemantic::IndexedSemantic(const IndexedSemantic& rhs_a, int index_a)
    ...
    void IndexedSemantic::MakeUserDefined(const std::string& semantic_name_a)
    {
...
#undef CASE_TO_STRING
    ...
    std::string TexAddressModeToString(const Reflection::TextureAddressMode t_a)
    {
        return TypeToStringSecondary(DetailsMap1::g_map_tex_address_mode_,
                                     t_a,
    ...
    Reflection::TextureAddressMode StringToTexAddressMode(
            const std::string& s_a)
    {
        return StringToTypeSecondary(DetailsMap1::g_map_tex_address_mode_,
                                     s_a,
                                     "SamplerState::TextureAddressMode");
    }
...
#undef CASE_TO_STRING
    ...
    Reflection::ResourceType BufferTypeToResourceType(const BufferType t_a)
    {
    ...
    Reflection::ResourceType SamplerTypeToResourceType(const SamplerType t_a)
    {
