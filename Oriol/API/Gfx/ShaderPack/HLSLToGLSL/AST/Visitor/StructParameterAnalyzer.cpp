        ...
        if (NotVisited(ast_a))
        {
            ...
            auto InsideShaderIOStruct = [this]() -> bool {
                if (auto struct_decl_ = ActiveStructDecl())
                    return struct_decl_->flags_(StructDecl::IsShaderInput
            ...
            if (!InsideShaderIOStruct())
            {
                    ...
                    || shader_target_ == ShaderTarget::GeometryShader)
                {
                                                    ...
                                                    ->GetStructDeclRef())
                    {
                        if (!IsActiveFunctionDeclEntryPoint()
                            || !IsVarEntryPointIO(ast_a)
                            || struct_decl_->HasMultipleShaderOutputInstances())
                        {
                            if (!IsVariableAnEntryPointParameter(decl_stmnt_))
                            {
    ...
    IMPLEMENT_VISIT_PROC(FunctionDecl)
    {
        if (NotVisited(ast_a))
        {
            if (!ast_a->flags_(FunctionDecl::IsEntryPoint)
                || shader_target_ == ShaderTarget::GeometryShader)
            {
                if (auto struct_decl_ = ast_a->return_type_->GetStructDeclRef())
                {
                    if (!ast_a->flags_(FunctionDecl::IsEntryPoint)
                        || struct_decl_->HasMultipleShaderOutputInstances())
                    {
