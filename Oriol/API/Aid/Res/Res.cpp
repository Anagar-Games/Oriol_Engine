...
namespace OL
{
    ...
    void ResWriter::AddSection(const std::string& section_a)
    {
        if (!sections_.count(section_a))
            sections_.emplace(section_a, std::vector<FileEntry> {});
    }

    void ResWriter::AddFile(const std::string& section_a,
    ...
    void ResWriter::RemoveFile(const std::string& section_a,
                               const std::string& name_a)
    {
        ...
        vec_.erase(std::remove_if(vec_.begin(),
                                  vec_.end(),
                                  [&](auto& e) { return e.name_ == name_a; }),
                   vec_.end());
    }

    void ResWriter::UpdateFile(const std::string& section_a,
                               const std::string& name_a,
    ...
    std::vector<std::string> ResWriter::ListSections() const
    {
    ...
    std::vector<std::string> ResWriter::ListFiles(
    ...
    void ResWriter::WriteToFile(const std::string& out_file_path_a) const
    {
        ...
        for (auto& [sec_name_, files_] : sections_)
        {
            ...
            for (auto& f_ : files_)
            {
                ...
                out_.write(reinterpret_cast<const char*>(&sz_), sizeof(sz_));
                out_.write(f_.data_.data(), sz_);
            }
        }
    }

    ResReader::ResReader(const std::string& res_file_path_a)
    ...
    std::vector<std::string> ResReader::ListSections() const
    {
        std::vector<std::string> out_;
    ...
    std::vector<std::string> ResReader::ListFiles(
            const std::string& section_a) const
    {
    ...
    bool ResReader::HasFile(const std::string& section_a,
    ...
    std::vector<char> ResReader::GetFile(const std::string& section_a,
                                      const std::string& name_a) const
    {
        auto sit_ = index_.find(section_a);
        if (sit_ == index_.end())
    ...
    void ResReader::EnsureDir(const std::string& full_path_a)
    {
        fs::path p_(full_path_a);
        fs::create_directories(p_.parent_path());
    }

    void ResReader::ExtractAll(const std::string& out_dir_a) const
    {
