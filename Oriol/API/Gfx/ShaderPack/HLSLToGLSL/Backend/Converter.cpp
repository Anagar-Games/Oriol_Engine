...
namespace OL
{
    ...
    bool Converter::FetchFromCurrentScope(const std::string& ident_a) const
    {
    ...
    {
        return (self_param_stack_.empty() ? nullptr : self_param_stack_.back());
    }
    ...
    void Converter::VisitScopedStmntList(std::vector<StmntPtr>& stmnt_list_a,
                                         void* args_a)
    {
    ...
    void Converter::MoveNestedStructDecls(std::vector<StmntPtr>& local_stmnts_a,
                                          bool global_scope_a)
    {
        for (auto it_ = local_stmnts_a.begin(); it_ != local_stmnts_a.end();)
        {
            if (auto var_decl_stmnt_ = (*it_)->As<VarDeclStmnt>())
            {
                ...
                {
                    auto struct_decl_stmnt_ = ASTFactory::MakeStructDeclStmnt(
                            ExchangeWithNull(var_decl_stmnt_->type_specifier_
                                                     ->struct_decl_));

                    InsertStmntBefore(struct_decl_stmnt_, global_scope_a);
                }
            } else if (auto basic_decl_stmnt_ = (*it_)->As<BasicDeclStmnt>())
            {
            ...
            const TypeDenoterPtr& type_denoter_a) const
    {
        if (type_denoter_a)
        {
            if (auto sampler_type_den_ =
                        type_denoter_a->GetAliased().As<SamplerTypeDenoter>())
            {
    ...
    void Converter::StmntScopeHandler::InsertStmntAfter(const StmntPtr& stmnt_a)
    {
        EnsureStmntList();
        InsertStmntAt(stmnt_a, idx_);
    }
    ...
    void Converter::StmntScopeHandler::InsertStmntAt(const StmntPtr& stmnt_a,
                                                     std::size_t pos_a)
    {
        if (stmnt_list_)
        {
