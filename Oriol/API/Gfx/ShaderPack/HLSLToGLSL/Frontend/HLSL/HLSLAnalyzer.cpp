...
namespace OL
{
    ...
    HLSLAnalyzer::HLSLAnalyzer(Log* log_a) : 
		Analyzer {log_a} {}

    void HLSLAnalyzer::DecorateASTPrimary(Program& program_a,
...
#define IMPLEMENT_VISIT_PROC(AST_NAME)                                         \
    ...
    IMPLEMENT_VISIT_PROC(Program)
    {
             ...
             ++it_)
        {
            ...
            {
                for (auto it_sub_ = uniform_buffer_decl_->local_stmnts_.begin();
    ...
    IMPLEMENT_VISIT_PROC(StructDecl)
    {
        ...
        for (const auto& member_ : ast_a->var_members_)
        {
            ...
            if (type_specifier_->IsConstOrUniform()
                ...
                Error(R_InvalidTypeModifierForMemberField, type_specifier_);
        }
    ...
    IMPLEMENT_VISIT_PROC(FunctionDecl)
    {
        ...
        {
            AnalyzeArrayDimensionList(param_->var_decls_.front()->array_dims_);
    ...
    IMPLEMENT_VISIT_PROC(UniformBufferDecl)
    {
        ...
        if (ast_a->slot_registers_.size() > 1)
            Error(R_BufferCanOnlyHaveOneSlot, ast_a->slot_registers_[1].get());

        for (const auto& slot_register_ : ast_a->slot_registers_)
        {
            if (slot_register_->shader_target_ != ShaderTarget::Undefined)
                Error(R_UserCBuffersCantBeTargetSpecific, slot_register_.get());
        }
    ...
    IMPLEMENT_VISIT_PROC(ForLoopStmnt)
    {
        ...
        if (WarnEnabled(Warnings::DeclarationShadowing))
        {
            ...
            {
                if (auto symbol_ = FetchFromCurrentScopeOrNull(it_.second))
                {
    ...
    IMPLEMENT_VISIT_PROC(ReturnStmnt)
    {
        ...
        if (ast_a->expr_)
        {
            ...
            if (auto expr_type_den_ = GetTypeDenoterFrom(ast_a->expr_.get()))
            {
                if (return_type_den_)
...
#undef IMPLEMENT_VISIT_PROC
                                           ...
                                           bool register_var_ident_a)
    {
        ...
        if (var_decl_a->initializer_)
        {
            ...
            if (var_decl_a->struct_decl_ref_)
                Error(R_MemberVarsCantHaveDefaultValues(var_decl_a->ToString()),
    ...
    void HLSLAnalyzer::AnalyzeVarDeclStaticMember(VarDecl* var_decl_a)
    {
        if (InsideGlobalScope())
            ...
            Error(R_StaticMembersCantBeDefinedInGlob(var_decl_a->ToString()),
                    ...
                    var_decl_a->namespace_expr_.get()))
        {
                        ...
                        type_den_->GetAliased().As<StructTypeDenoter>())
            {
                                                   ...
                                                   var_decl_a))
                {
                    ...
                    {
                        ReportHandler::HintForNextReport(R_PrevDefinitionAt(
                                prev_var_def_->area_.Pos().ToString()));

                        Error(R_StaticMemberVarRedef(var_decl_a->ToString()),
                              var_decl_a);
                    } 
                                                ...
                                                ->type_specifier_.get()))
                            {
                                if (!decl_var_type_->Equals(*def_var_type_))
                                {
                                    ReportHandler::HintForNextReport(
                                            R_DeclaredAt(member_var_decl_->area_
                                                                 .Pos()
                                                                 .ToString()));

                                    Error(R_DeclTypeDiffersFromDefType(
                                                  decl_var_type_->ToString(),
                                                  def_var_type_->ToString()),
                                          var_decl_a);
                                } 
            ...
            const TypeDenoter* prefix_type_denoter_a)
    {
        PushCallExpr(call_expr_a);
        {
            ...
            {
                auto intr_it_ = HLSLIntrinsicAdept::GetIntrinsicMap().find(
                        call_expr_a->ident_);
                if (intr_it_ != HLSLIntrinsicAdept::GetIntrinsicMap().end())
                {
            ...
            call_expr_a->ForEachOutputArgument(
                    [this](ExprPtr& arg_expr_, VarDecl* param_) {
                                           ...
                                           call_expr_a))
            {
                if (is_static_a)
                {
                    if (!func_decl_->IsStatic())
                        Error(R_IllegalStaticFuncCall(func_decl_->ToString()),
                              call_expr_a);
                } 
				else
                {
            ...
            const BufferType buffer_type_a)
    {
		...
		else if (IsStreamBufferType(buffer_type_a))
        {
            if (IsStreamOutputIntrinsic(intrinsic_))
            {
                ...
                Error(R_InvalidIntrinsicForStreamOutput(ident_), call_expr_a);
        }
        ...
        if (IsTextureGatherIntrisic(intrinsic_))
        {
            ...
            case BufferType::TextureCubeArray:
                ...
                break;
            default:
                Error(R_InvalidClassIntrinsicForType(ident_,
                                                     BufferTypeToString(
                                                             buffer_type_a)),
                      call_expr_a);
                break;
    ...
    void HLSLAnalyzer::AnalyzeIntrinsicWrapperInlining(CallExpr* call_expr_a)
    {
    ...
    void HLSLAnalyzer::AnalyzeObjectExpr(ObjectExpr* expr_a, PrefixArgs* args_a)
    {
        if (expr_a->prefix_expr_)
        {
            ...
            if (expr_a->symbol_ref_)
            {
                if (auto var_decl_ = expr_a->symbol_ref_->As<VarDecl>())
                {
                    if (!var_decl_->IsStatic())
                    {
                        ...
                        {
                            if (base_struct_decl_a
                                        ->IsBaseOf(active_struct_decl_, true))
                                return;
                        }
            ...
            bool is_static_a)
    {
        if (type_specifier_a)
        {
            if (type_specifier_a->HasAnyStorageClassOf({StorageClass::Static}))
            {
                if (!is_static_a)
                {
                    Error(R_IllegalNonStaticAccessToMember(ident_a), expr_a);
                    return false;
                }
            } else
            {
        ...
        for (auto& array_index_ : expr_a->array_indices_)
        {
            if (auto type_den_ = GetTypeDenoterFrom(array_index_.get()))
            {
                            ...
                            type_den_aliased_.As<BaseTypeDenoter>())
                {
                    ...
                    const auto int_vec_type_ = VectorDataType(DataType::Int,
                                                              vector_dim_);
        ...
        if (WarnEnabled(Warnings::IndexBoundary))
        {
                        ...
                        expr_a->prefix_expr_.get()))
            {
                                    ...
                                    .As<ArrayTypeDenoter>())
                {
                    for (std::size_t
                                 i_ = 0,
                                 n_ = std::min(expr_a->array_indices_.size(),
                                               prefix_array_type_den_
                                                       ->array_dims_.size());
                         i_ < n_;
                         ++i_)
                    {
                                    ...
                                    *(expr_a->array_indices_[i_])))
                        {
                            try
                            {
                                ...
                                prefix_array_type_den_->array_dims_[i_]
                                        ->ValidateIndexBoundary(array_idx_);
                            } catch (const std::exception& e_)
                            {
                                Warning(e_.what(),
    ...
    void HLSLAnalyzer::AnalyzeEntryPoint(FunctionDecl* func_decl_a)
    {
        if (func_decl_a->flags_.SetOnce(FunctionDecl::IsEntryPoint))
        {
            ...
            AnalyzeEntryPointAttributes(func_decl_a->decl_stmnt_ref_->attribs_);
        }
    }

    void HLSLAnalyzer::AnalyzeEntryPointInputOutput(FunctionDecl* func_decl_a)
    {
        for (auto& param_ : func_decl_a->parameters_)
        {
        ...
        {
            if (it_.second > 1)
                Error(R_DuplicateUseOfOutputSemantic(it_.first.ToString()));
        }
    }

    void HLSLAnalyzer::AnalyzeEntryPointParameter(FunctionDecl* func_decl_a,
                                                  VarDeclStmnt* param_a)
    {
        ...
        {
            if (param_a->IsOutput())
                AnalyzeEntryPointParameterInOut(func_decl_a, var_decl_, false);

            else if (param_a->IsInput())
                AnalyzeEntryPointParameterInOut(func_decl_a, var_decl_, true);
        }
    }
            ...
            bool input_a)
    {
        if (struct_decl_a)
        {
            ...
            if (input_a)
                struct_decl_a->AddFlagsRecursive(StructDecl::IsShaderInput);
            else
                struct_decl_a->AddFlagsRecursive(StructDecl::IsShaderOutput);
        }
    }
            ...
            const std::vector<AttributePtr>& attribs_a)
    {
        ...
        ErrorIfAttributeNotFound(found_partitioning_, "partitioning(mode)");
            ...
            const std::vector<AttributePtr>& attribs_a)
    {
        ...
        ErrorIfAttributeNotFound(found_num_threads_, "numthreads(x, y, z)");
    }
...
#define COMMON_SEMANTICS_EX COMMON_SEMANTICS, T::ClipDistance, T::CullDistance
        ...
        case ShaderTarget::VertexShader:
            ...
            {
                ValidateInSemantics({COMMON_SEMANTICS});
                ValidateOutSemantics(
                        {COMMON_SEMANTICS_EX, T::VertexPosition, T::PointSize});
            }
            break;
...
#undef COMMON_SEMANTICS_EX
    ...
    void HLSLAnalyzer::AnalyzeSecondaryEntryPoint(FunctionDecl* func_decl_a,
                                                  bool is_patch_constant_func_a)
    {
        if (func_decl_a->flags_.SetOnce(FunctionDecl::IsSecondaryEntryPoint))
        {
            ...
            const std::vector<AttributePtr>& attribs_a)
    {
        ...
        {
        case ShaderTarget::TessellationEvaluationShader:
            AnalyzeSecondaryEntryPointAttributesTessEvaluationShader(attribs_a);
            break;
        default:
            break;
        }
    ...
    void HLSLAnalyzer::AnalyzeSecondaryEntryPointAttributesTessEvaluationShader(
            const std::vector<AttributePtr>& attribs_a)
    {
        for (const auto& attr_ : attribs_a)
        {
                                              ...
                                              bool required_a)
    {
        ...
        {
            AnalyzeAttributeValue(attrib_a->arguments_[0].get(),
    ...
    void HLSLAnalyzer::AnalyzeAttributeOutputControlPoints(Attribute* attrib_a)
    {
    ...
    void HLSLAnalyzer::AnalyzeAttributePatchConstantFunc(Attribute* attrib_a)
    {
        if (AnalyzeNumArgsAttribute(attrib_a, 1))
        {
            auto literal_expr_ = attrib_a->arguments_[0]->As<LiteralExpr>();
            if (literal_expr_ && literal_expr_->data_type_ == DataType::String)
            {
                auto literal_value_ = literal_expr_->GetStringValue();
                if (auto patch_const_func_ = FetchFunctionDecl(literal_value_))
                {
                    AnalyzeSecondaryEntryPoint(patch_const_func_, true);
                } else
                    Error(R_EntryPointForPatchFuncNotFound(literal_value_),
                          attrib_a->arguments_[0].get());
            } else
    ...
    void HLSLAnalyzer::AnalyzeAttributeNumThreadsArgument(Expr* expr_a,
        ...
        if (!var_decl_sm3_semantics_.empty())
        {
            for (auto var_decl_ : var_decl_sm3_semantics_)
                AnalyzeSemanticSM3(var_decl_->semantic_,
                                   var_decl_->flags_(VarDecl::IsShaderInput));
