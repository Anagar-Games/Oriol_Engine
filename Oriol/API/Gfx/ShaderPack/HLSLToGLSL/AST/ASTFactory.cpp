...
namespace OL
{
    namespace ASTFactory
    {
                ...
                const ExprPtr& sampler_object_expr_a)
        {
            auto ast_ = MakeAST<CallExpr>();
            {
                const auto& type_den_ =
                        texture_object_expr_a->GetTypeDenoter()->GetAliased();
                if (auto buffer_type_den_ = type_den_.As<BufferTypeDenoter>())
                {
                    ast_->type_denoter_ = std::make_shared<SamplerTypeDenoter>(
        ...
        CallExprPtr MakeWrapperCallExpr(const std::string& func_ident_a,
                                        const TypeDenoterPtr& type_denoter_a,
                                        const std::vector<ExprPtr>& arguments_a)
        {
            auto ast_ = MakeAST<CallExpr>();
        ...
        CastExprPtr MakeLiteralCastExpr(const TypeDenoterPtr& type_denoter_a,
        ...
        TypeSpecifierPtr MakeTypeSpecifier(const StructDeclPtr& struct_decl_a)
        {
        ...
        TypeSpecifierPtr MakeTypeSpecifier(const TypeDenoterPtr& type_denoter_a)
        {
            auto ast_ = MakeAST<TypeSpecifier>();
            {
                ast_->type_denoter_ = type_denoter_a;
        ...
        ArrayExprPtr MakeArrayExpr(
                ...
                const std::vector<ExprPtr>::const_iterator& array_indices_end_a)
        {
            auto ast_ = MakeAST<ArrayExpr>();
            {
                ast_->prefix_expr_ = prefix_expr_a;
        ...
        RegisterPtr MakeRegister(int slot_a, const RegisterType register_type_a)
        {
                ...
                const TypeDenoterPtr& type_den_a)
        {
            if (auto struct_type_den_ = type_den_a->As<StructTypeDenoter>())
            {
                if (auto struct_decl_ = struct_type_den_->struct_decl_ref_)
                {
                    ...
                    return MakeCastExpr(
                            type_den_a,
                            MakeConstructorListExpr(expr_a, member_type_dens_));
                }
            } else if (auto base_type_den_ = type_den_a->As<BaseTypeDenoter>())
            {
        ...
        static ExprPtr MakeConstructorListExprPrimary(
                const ExprPtr& expr_a,
                std::vector<TypeDenoterPtr>::const_iterator type_dens_begin_a,
                std::vector<TypeDenoterPtr>::const_iterator type_dens_end_a)
        {
            if (type_dens_begin_a + 1 != type_dens_end_a)
            {
                auto ast_ = MakeAST<SequenceExpr>();
                {
                    ...
                    ast_->Append(
                            MakeConstructorListExprPrimary(expr_a,
                                                           type_dens_begin_a
                                                                   + 1,
                                                           type_dens_end_a));
                }
                ...
                const std::vector<TypeDenoterPtr>& list_type_dens_a)
        {
            if (list_type_dens_a.empty())
                ...
                return MakeConstructorListExprPrimary(expr_a,
        ...
        {
            return MakeAssignStmnt(MakeArrayExpr(MakeObjectExpr(var_decl_a),
                                                 array_indices_a),
                                   assign_expr_a);
        }
                ...
                const std::vector<int>& array_indices_a)
        {
            ...
            for (auto index_ : array_indices_a)
                exprs_.push_back(
                        MakeLiteralExpr(DataType::Int, std::to_string(index_)));
                                    ...
                                    const ExprPtr& sub_expr_a)
        {
            ...
            {
                auto ast_ = std::static_pointer_cast<LiteralExpr>(sub_expr_a);
                {
        ...
        ArrayDimensionPtr ConvertExprToArrayDimension(const ExprPtr& expr_a)
        {
            auto ast_ = MakeAST<ArrayDimension>();
            {
                if (expr_a)
        ...
        std::vector<ArrayDimensionPtr> ConvertExprListToArrayDimensionList(
                const std::vector<ExprPtr>& exprs_a)
        {
            std::vector<ArrayDimensionPtr> array_dims_;
