...
namespace OL
{
            ...
            bool enable_warnings_a)
    {
        ...
        {
            ParseProgram();
            return (GetReportHandler().HasErrors() ? nullptr
                                                   : std::move(output_));
                                           ...
                                           int int_value_a)
    {
        auto ident_tkn_ = std::make_shared<Token>(SourcePosition::ignore,
                                                  Token::Types::Ident,
                                                  ident_a);
        auto value_tkn_ = std::make_shared<Token>(SourcePosition::ignore,
        ...
        DefineMacro({ident_tkn_, value_token_string_, {}, false, true, false});
    }
    ...
    Variant PreProcessor::EvaluateExpr(const TokenPtrString& token_string_a,
                                       const Token* tkn_a)
    {
        Variant value_;
                ...
                value_ = expr_avaluator_.Evaluate(
                        *condition_expr_,
                        [](ObjectExpr* expr_) -> Variant {
                            RuntimeErr(R_UndeclaredIdent(expr_->ident_), expr_);
                            return {};
                        });
    ...
    {
        return (if_block_stack_.empty() ? IfBlock() : if_block_stack_.top());
    }
            ...
            const std::vector<TokenPtrString>& arguments_a)
    {
        ...
        auto ExpandTokenString =
            ...
            case Tokens::Directive: {
                auto ident_ = tkn_.Spell();
                for (std::size_t i = 0; i < macro_a.parameters_.size(); ++i)
                {
    ...
    void PreProcessor::ParseProgram()
    {
        ...
        while (!if_block_stack_.empty())
        {
            ...
            GetReportHandler().SubmitReport(false,
                                            ReportTypes::Error,
                                            R_SyntaxError,
            ...
            if (it_ != macros_.end())
            {
                ...
                if (macro_.HasParameterList())
                {
                    token_string_.PushBack(
                            ParseIdentArgumentsForMacro(ident_tkn_, macro_));
                ...
                && arguments_.size() < macro_a.parameters_.size()))
        {
            ...
            } else
            {
                ...
                if (arguments_.size() > macro_a.parameters_.size())
                    error_msg_ =
                            R_TooManyArgsForMacro(ident_token_a->Spell(),
                                                  macro_a.parameters_.size(),
                                                  arguments_.size());

                if (arguments_.size() < macro_a.parameters_.size())
                    error_msg_ =
                            R_TooFewArgsForMacro(ident_token_a->Spell(),
                                                 macro_a.parameters_.size(),
                                                 arguments_.size());
        ...
        if (Is(Tokens::LBracket))
        {
            ...
            if (!Is(Tokens::RBracket))
            {
                while (true)
                {
                    ...
                    {
                        auto param_ident_ = Accept(Tokens::Ident)->Spell();
                        IgnoreWhiteSpaces();
    ...
    void PreProcessor::ParseDirectiveIfOrElifCondition(bool is_else_branch_a,
            ...
            if ((*token_it_)->Type() == Tokens::Ident)
            {
                ...
                } else if (command_ == "pack_matrix")
                {
                    ...
                    try
                    {
                        ...
                        auto alignment_tkn_ = token_it_.Accept(Tokens::Ident);
                        ...
                        if (alignment_ == "row_major"
                            ...
                            Warning(R_UnknownMatrixPackAlignment(alignment_),
                                    alignment_tkn_.get());
                    } catch (const std::exception& e_)
                    {
                ...
                {
                    Warning(R_PragmaCantBeHandled(command_), token_it_->get());
                    return;
                } else
            ...
            bool ignore_comments_a)
    {
        ...
        while (!Is(Tokens::NewLine))
        {
            ...
            case Tokens::Ident: {
                if (expand_defined_directive_a && Tkn()->Spell() == "defined")
                {
                    auto defined_macro_ = ParseDefinedMacro();
                    token_string_.PushBack(
                            Make<Token>(Tokens::IntLiteral, defined_macro_));
                } else
                {
