...
#ifndef OL_ANALYZER_HPP
#define OL_ANALYZER_HPP
...
namespace OL
{
    ...
    protected:
        ...
        virtual void DecorateASTPrimary(Program& program_a,
                                        const ShaderInput& input_desc_a,
                                        const ShaderOutput& output_desc_a) = 0;

        void SubmitReport(bool is_error_a,
        ...
        AST* FetchFromCurrentScopeOrNull(const std::string& ident_a) const;

        Decl* FetchDecl(const std::string& ident_a, const AST* ast_a = nullptr);

        Decl* FetchType(const std::string& ident_a, const AST* ast_a = nullptr);
        ...
        void AnalyzeTypeSpecifierForParameter(TypeSpecifier* type_specifier_a);
